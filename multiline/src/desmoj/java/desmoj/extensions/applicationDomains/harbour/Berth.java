 ...
 * and/or loaded. <code>Ship</code> can come and <code>take()</code> the
 * needed length for his berthing. Each ship has to release the same length of
 * the berth it once has acquired by calling the <code>release()</code> method
 ...
 * After the berht has <code>release()</code> the used length the waiting
 ...
 *
 */
public class Berth extends desmoj.core.simulator.QueueBased {
	 ...
	 * and the capacity limit of the underlying queue can be chosen.
	 * 
	 * @param owner
	 *            Model : The model this Berth is associated to.
	 * @param name
	 *            java.lang.String : The Berth's name
	 * @param sortOrder
	 ...
	 *            (ships) can be enqueued. Zero (0) means unlimited capacity.
	 * @param length
	 *            int : The length of the Berth. Must be positive and greater
	 *            than 0.
	 * @param showInReport
	 *            boolean : Flag, if Berth should produce a report or not.
	 * @param showInTrace
	...
	/**
	 * Constructor for a Berth with a certain length. The underlying queue has a
	 * Fifo queueing discipline and unlimited capacity.
	 * 
	 * @param owner
	 *            Model : The model this Berth is associated to.
	 * @param name
	...
	/**
	 * Changes the length of the Berth. Sets the length of the berth that is
	 * max. available to l. l must be positive. This is only allowed as long as
	 * the Berth has not been used or the Berth has just been reset.
	 * 
	 * @param l
	 *            int : The new length of the Berth. Must be positive.
	 */
	...
	/**
	 * Returns the average usage of the Berth. That means: in average, which
	 * percentage of the time it was in use over the time?
	 * 
	...
	public double avgUsage() {
		...
		double result = SumUT / now.getTimeValue();
		return result;
	}

	/**
	 * Provides the length to ships to use it for its berthing. As not enough
	 * length is available at the moment the Ship has to wait in a queue until
	 * enough length is available again.
	 * 
	 * @return boolean : Is <code>true</code> if the needed length has been
	 *         provided successfully, <code>false</code> otherwise (i.e.
	 ...
	 * 
	 * @param l
	 *            int : Is positive when the Berth <code>release()</code> some
	 *            length and negative when the Berth <code>take()</code> some
	 *            length.
	 */
	...
	/**
	 * Activates the first process (ship) waiting in the queue. That is a
	...
	/**
	 * Resets the statistics of this Berth. The number of available length at
	 * this moment and the ships waiting in the queue are not changed. But all
	 * statistic counters are reset. The parent <code>QueueBased</code> is
	 * also reset.
	 */
	public void reset() {
	 ...
	 * 
	 * @return long : The number of Users. That are ships have acquired and
	 *         released the needed length of the berth.
	 */
