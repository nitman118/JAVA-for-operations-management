...
/**
 * An entity is a basic-object of animation. Every entity has an entity-type. 
 * Basic-properties of an entity e.g. possible attributes or states are definied 
 * in entity-type.
 * The history of all values are stored also.  
 * All grafical properies are stored in EntityGrafic
 * 
...
public class Entity implements Basic{
	 ...
	 */
	private Hashtable<String, List<Attribute>>attribute;
	/**
	...
	public Entity(Model model, String id, String entityTypeId , String state, long time) throws ModelException{
		...
		if(this.id != null) model.getEntities().add(this);
	}
	...
	public void setState(String state, long time) throws ModelException{
		if(this.entityTyp.existPosibleState(state)){
			Attribute last = null;
			try{ last = this.state.get(this.state.size()-1);
	 ...
	 * This history is stored in containerHistory
	...
	public boolean isFree(){
		...
		String[] p = desmoj.extensions.visualization2d.engine.command.Parameter.split(key);
	...
	public String getAttribute(String key){
		...
		if(v != null)	out = v.get(v.size()-1).getValue();
		return out;
	}
	 ...
	 * @throws ModelException
	 */
	public Grafic createGraficStatic(String viewId, double positionX, double positionY, double angle, boolean direction, long time)throws ModelException{
