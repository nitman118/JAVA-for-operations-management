...
public class MyTableUtilities {
	...
	class MyTableModel implements TableModel{
		...
		public MyTableModel(JComponent [][] comps){
			...
			this.cols	= 0;
			if(this.rows > 0) this.cols = comps[0].length;
			boolean flag = true;
			for(int i=0; i<this.rows; i++){
				if(comps[i].length != this.cols) flag = false;
			}
			if(! flag)
		...
		public Object getValueAt(int row, int col) {
			...
			if(row >=0 && col >= 0){
				Object out;
				if(horizontal) 	out = this.comps[row][col];
				else			out = this.comps[col][row];
				return out;
			}
			else
				return null;
