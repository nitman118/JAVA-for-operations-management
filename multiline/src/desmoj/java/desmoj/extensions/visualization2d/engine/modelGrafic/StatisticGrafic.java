...
public class StatisticGrafic extends JComponent 
	...
	public static Rectangle getBoundsExternGlobal(Model model, String viewId){
		...
		Rectangle r = null;
		if(found) r = new Rectangle((int)Math.round(minX), (int)Math.round(minY), (int)Math.round(maxX-minX), (int)Math.round(maxY-minY));
	...
	public StatisticGrafic(Statistic statistic, String viewId, Point pointExtern, 
			int typeAnimation, boolean isIntValue, Dimension deltaSize, boolean infopane) throws ModelException {
		...
		if(this.deltaSize == null) this.deltaSize = new Dimension(0,0); 
	...
	private JPanel buildLastValuePanel(){
		...
		case Statistic.INDEX_None:
			...
			if(this.isIntValue) out.setPreferredSize(new Dimension(80,50));
			else 				out.setPreferredSize(new Dimension(100,50));
			break;
		case Statistic.INDEX_Min_Max:
			...
			if(this.isIntValue) out.setPreferredSize(new Dimension(80,70));
			else 				out.setPreferredSize(new Dimension(100,70));
			break;
	 ...
	 * Will be called by Statistic.update(....).
	 * @throws ModelException
	 */
	...
	private void setCode(){
		if(this.statistic.getName() != null) 	this.code = this.statistic.getName();
		else 								this.code = "id: "+this.statistic.getId();
	}
	...
	private void buildTimeValueDiagramAxisFormat(XYPlot plot, String rangeAxisLabel){
		...
		String format, unit;
		if( diff > 24*60*60*1000) 	{format = "d.MM.yyyy"; 	unit="[day]";}
		else if (diff > 60*60*1000) {format = "H:mm"; 		unit="[h]";}
		else if (diff > 60*1000) 	{format = "m:ss"; 		unit="[min]";}
		else if (diff > 1000) 		{format = "s.S"; 		unit="[sec]";}
		else 						{format = "S"; 			unit="[millisec]";}
	...
	private String formatieren(double a, boolean isIntValue){
		...
		if(isIntValue){
			a = Math.rint(a);
			if(Math.abs(a) < 10000.0)		format = "#0";
			else							format = "0.E0";
		}else{
			if(Math.abs(a) < 0.001) 		format = "0.00E0";
			else if(Math.abs(a) < 10000.0) 	format = "#0.000";
			else							format = "0.00E0";
		}
	...
	public void actionPerformed(ActionEvent event){
		...
		if( infoPane != null){
			...
			if(StatisticGrafic.TEXT_POPUP_MENU[0].equals(event.getActionCommand())){
				infoPane.addStatistic(this.statistic.getId(), code, StatisticGrafic.ANIMATION_TimeValueDiagram, isIntValue);
	...
	public void mouseClicked(MouseEvent event){this.checkPopupMenu(event);}
	public void mouseEntered(MouseEvent event){}
	public void mouseExited(MouseEvent event){}
	public void mousePressed(MouseEvent event){this.checkPopupMenu(event);}
	public void mouseReleased(MouseEvent event){this.checkPopupMenu(event);}
