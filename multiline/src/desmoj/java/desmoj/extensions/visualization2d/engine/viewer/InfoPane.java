...
public class InfoPane extends JTabbedPane implements ActionListener, MouseListener{
	...
	public void removeEntry(int j){
		...
		if(this.content.isEmpty()) 	this.setVisible(false);
		else{
			this.updateTab();
	 ...
	 * Wird von EntityGrafic actionPerformed aufgerufen 
	...
	public void addEntity(String entityId, String code){
		...
		for(int i=1; i<this.content.size(); i++){
			...
			if(ic.equals(nc)) this.content.remove(i);
		}
		this.updateTab();
		this.setSelectedIndex(0);
		this.setPreferredSize(getSize());
	 ...
	 * Wird von StatisticGrafic actionPerformed aufgerufen 
	 ...
	 * Damit die zuvor ausgewaehlten Statistiken weiterverwendet werden koennen,
	 * muessen die Grafikobjekte neu erstellt werden.
	 * Alle EntityObjekte werden geloescht, das sie veraltet sind.
	 */
	public void refresh(){
	...
	public void mouseReleased(MouseEvent e) {
		...
		if(this.moveClick != null) mc = this.moveClick;
		this.moveClick = null;
	...
	class InfoContent{
		...
		public void refresh(){
			if(! this.entity){
				...
				this.statistikComp			= (StatisticGrafic)statistic.createGrafic(null, 0,0, statistikTyp, statistikIsIntValue, null, true);
	...
	class TabContent extends JPanel{
		...
		public TabContent(String entityId){
			...
			columNames.add("Key"); columNames.add("Value");columNames.add("Since");
			...
			if(model.getEntities().exist(entityId)){
				...
				JTable table	= new JTable(rowData, new java.util.Vector<String>(columNames));
		...
		private void createContent(String entityId){
			...
			row.add("Id"); row.add(entity.getId()); row.add("");
			rowData.add(row); headerRows.add(new Integer(i++));
			row = new Vector<String>();
			row.add("EntityTypeId"); row.add(entityType.getId()); row.add("");
			rowData.add(row); i++;
			v = entity.getStateHistory();
			for(int k=v.size()-1; k>-1; k--){
				row = new Vector<String>();
				row.add("State ("+k+")"); row.add(v.get(k).getValue()); row.add(SimulationTime.getTimeString(v.get(k).getSince(),SimulationTime.SHOW_DATE_TIME));
				if(k<v.size()-1) oldValueRows.add(new Integer(i));
				rowData.add(row); i++;
			}
			v = entity.getNameAttribute();
			for(int k=v.size()-1; k>-1; k--){
				row = new Vector<String>();
				row.add("NameAttr ("+k+")"); row.add(v.get(k).getValue()); row.add(SimulationTime.getTimeString(v.get(k).getSince(),SimulationTime.SHOW_DATE_TIME));
				if(k<v.size()-1) oldValueRows.add(new Integer(i));
				rowData.add(row); i++;
			}
			...
			for(int k=v.size()-1; k>-1; k--){
				row = new Vector<String>();
				row.add("PriorityAttr ("+k+")"); row.add(v.get(k).getValue()); row.add(SimulationTime.getTimeString(v.get(k).getSince(),SimulationTime.SHOW_DATE_TIME));
				if(k<v.size()-1) oldValueRows.add(new Integer(i));
				rowData.add(row); i++;
			}
			...
			for(int k=v.size()-1; k>-1; k--){
				row = new Vector<String>();
				row.add("VelocityAttr ("+k+")"); row.add(v.get(k).getValue()); row.add(SimulationTime.getTimeString(v.get(k).getSince(),SimulationTime.SHOW_DATE_TIME));
				if(k<v.size()-1) oldValueRows.add(new Integer(i));
				rowData.add(row); i++;
			}
			row = new Vector<String>();
			row.add("History:"); row.add(""); row.add("");
			rowData.add(row); headerRows.add(new Integer(i++));
			...
			for(int k=v.size()-1; k>-1; k--){
				...
				if(! (ky[1].equals("free"))||(ky[1].equals("static"))){
					row = new Vector<String>();
					row.add(ky[1]+"  ("+ky[0]+")"); row.add(v.get(k).getValue()); row.add(SimulationTime.getTimeString(v.get(k).getSince(),SimulationTime.SHOW_DATE_TIME));
					rowData.add(row); i++;
				}
			}

			row = new Vector<String>();
			row.add("Attributes:"); row.add("");
			rowData.add(row); headerRows.add(new Integer(i++));
			...
			for(int j=0; j<posAttr.length; j++){
				...
				if(v==null){
					row = new Vector<String>();
					row.add(posAttr[j]); row.add(""); row.add("");
					rowData.add(row); oldValueRows.add(new Integer(i++));
				}
			}
			
			row = new Vector<String>();
			row.add("possible States:"); row.add("");
			rowData.add(row); headerRows.add(new Integer(i++));
			for(int j=0; j<posStates.length; j++){
				row = new Vector<String>();
				row.add(""); row.add(posStates[j]); row.add("");
				rowData.add(row); i++;
			}
			row = new Vector<String>();
	...
	class ColoredTableCellRenderer implements TableCellRenderer{
		...
		public Component getTableCellRendererComponent(JTable table,
			...
			if(headerRows.contains(new Integer(row))){
				...
				label.setText(begin+shift+(String)value+end);
