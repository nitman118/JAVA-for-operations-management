...
public class WaitQueueAnimation extends WaitQueue{
	...
	private int				masterSortOrder;
	private int				slaveSortOrder;
	private String			id;
	...
	public void createAnimation(Position pos, FormExt form,
		...
		if(this.showInAnimation){
			try {
				if(init)	c = Command.getCommandInit("createWaitQueue", this.cmdGen.getAnimationTime(simTime));
	 ...
	 * @return
	 */
	public boolean cooperate(ProcessCoopAnimation coop){
		SimProcess master = this.currentSimProcess();
		boolean out;
		TimeInstant simTime = this.getModel().presentTime();
		boolean	init	= this.cmdGen.isInitPhase();
		Command c;
		String[] insertMaster = {master.getName(), Integer.toString(master.getPriority()), this.masterPriorityAttribute};
		if(this.showInAnimation){
			try {
	 ...
	 * @return
	 */
	public boolean cooperate(ProcessCoopAnimation coop, Condition cond){
		SimProcess master = this.currentSimProcess();
		boolean out;
		TimeInstant simTime = this.getModel().presentTime();
		boolean	init	= this.cmdGen.isInitPhase();
	...
	public boolean waitOnCoop(){
		...
		boolean	init	= this.cmdGen.isInitPhase();
		Command c;
		String[] insertSlave = {slave.getName(), Integer.toString(slave.getPriority()), this.slavePriorityAttribute};
		if(this.showInAnimation){
			try {
				if(init)	c = Command.getCommandInit("setWaitQueue", this.cmdGen.getAnimationTime(simTime));
				else 		c = Command.getCommandTime("setWaitQueue", this.cmdGen.getAnimationTime(simTime));
