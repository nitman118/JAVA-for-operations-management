...
import desmoj.extensions.applicationDomains.production.RestockProcessQS;
 ...
 *
 */
public class RestockProcessQSAnimation extends RestockProcessAnimation{

	public RestockProcessQSAnimation(RestockProcessQS restock) {
		this.restock			= restock;
		this.check(restock);
	}
	
	private void check(RestockProcessQS restock){
		if (! (restock.getModel() instanceof ModelAnimation)) 
			throw new SimulationException("For animation, RestockProcessQS must be initialized with a model of type ModelAnimation");
	}
