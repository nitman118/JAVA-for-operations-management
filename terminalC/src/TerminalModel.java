...
public class TerminalModel extends Model { // Terminal Model class inherits (desmoj) Model
    ...
    ContDistExponential rushOrderShipInterarrivalTime; // desmoj declaration for determining
                                                       // exponential
    // distribution, for rush order ship inter arrival time
    ContDistUniform containerCountDistribution; // desmoj declaration for determining uniform
    // distribution, for container count
    ContDistUniform unloadTime; // desmoj declaration for determining uniform distribution,
    // for unload time
    ContDistUniform loadTime; // desmoj declaration for determining uniform distribution,
    // for load time
    ContDistUniform storageTime; // desmoj declaration for determining uniform distribution,
    // for storage time

    ProcessQueue<Container> inboundQueue; // desmoj queue of inbound container objects
    ProcessQueue<Container> rushOrderInboundQueue; // desmoj queue of inbound rush container
                                                   // objects
    ...
    public String description() { // description
        return "Model - Assignment (c)";
    }

    public void init() { // initialization method
        shipInterarrivalTime = new ContDistExponential(this, "ShipInterarrivalTime", 12, true,
                true); //
        // exponential distribution for ship inter-arrival time, format: ( 'this' model
        // as owner,
        // description, mean arrival = 12 hours, show in report, show in trace)
        shipInterarrivalTime.setNonNegative(true); // setting arrival time as non-negative
        // as is a possibility with exponential distribution

        rushOrderShipInterarrivalTime = new ContDistExponential(this,
                "rushOrderShipInterarrivalTime", 48, true, true); //
        // exponential distribution for rush order ship inter-arrival time, format: (
        // 'this' model
        // as owner, description, mean arrival = 12 hours, show in report, show in
        // trace)
        rushOrderShipInterarrivalTime.setNonNegative(true); // setting arrival time as
                                                            // non-negative
        // as is a possibility with exponential distribution

        containerCountDistribution = new ContDistUniform(this, "ContainerCount", 1.0, 100.0,
                true, true); //
        // Uniform distribution for container count, format: ( 'this' model as owner,
        // description,
        // minimum, maximum, show in report, show in trace)
        ...
        // Uniform distribution for Storage Time of containers, format: ( 'this' model
        // as owner, description, minimum, maximum, show in report, show in trace)
        ...
        rushOrderInboundQueue = new ProcessQueue<Container>(this, "RushInboundContainer", true,
                true); //
        // queue for rush inbound containers, format: ( this model object as owner,
        // description,
    ...
    public void doInitialSchedules() { // method for defining initial schedules
        ...
        RushOrderContainerGenerator rgenerator = new RushOrderContainerGenerator(this,
                "Rush Container Arrival", true); //
        // create new instance of rushOrderContainerGenerator, format ( owner model,
        // description, show in trace)
        rgenerator.activate(); // activate rgenerator life cycle
