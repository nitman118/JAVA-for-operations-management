...
public class Container extends SimProcess { // container class inherits SimProcess
    ...
    boolean isRush; // boolean to signify that container is a rush order
    ...
    public void lifeCycle() { // class method dealing with life-cycle of container
        if (!this.isRush) // if container is not rush
        {
            myModel.inboundQueue.insert(this); // add container to inbound queue

        }

        else { // if container is a rush order
            myModel.rushOrderInboundQueue.insert(this); // insert it into rush order inbound
                                                        // queue
        }

        if (!myModel.inboundCraneQueue.isEmpty()) { // if the crane queue for processing
            // inbound containers is not empty (i.e
            // there is an idle crane for
            // processing inbound containers)
            Crane crane = myModel.inboundCraneQueue.removeFirst(); // remove first idle
            // crane
            // from queue
            crane.isInbound = true; // set boolean to true to signify that the container
            // being processed is in inbound queue
        ...
        if (!myModel.outboundCraneQueue.isEmpty()) { // if outbound crane queue is not empty (
                                                     // i.e there is an idle crane for
                                                     // processing outbound containers)
