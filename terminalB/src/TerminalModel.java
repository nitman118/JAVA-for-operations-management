...
import java.util.List; // import List specific package
...
public class TerminalModel extends Model { // Terminal Model class inherits (desmoj) Model
    ...
    ContDistExponential shipInterarrivalTime; // desmoj declaration for determining
    // exponential distribution, for ship inter arrival time
    ContDistUniform containerCountDistribution; // desmoj declaration for determining
    // uniform distribution, for container count
    ...
    List<ContainerMeasurement> measurements;
    ...
    public TerminalModel(Model owner, String modelName, boolean showInReport, //
        ...
        measurements = new ArrayList<ContainerMeasurement>(); // define measurements
        // as new Array List
    }

    public String description() { // description
        return "Model - Assignment (b)";
    }
    ...
    public void init() { // initialization method
        ...
        inboundQueue = new ProcessQueue<Container>(this, "InboundContainer", true, true); // queue
                                                                                          // for
        // inbound containers, format: ( this model object as owner, description, show
        // in report,
        // show in trace)
    ...
    double computeAverageCycleTime(List<ContainerMeasurement> cs) { // method to compute
                                                                    // average cycle time

        double cycTime = 0; // initializing average cycle time

        for (ContainerMeasurement counter : cs) { // for all objects in cs
            cycTime += (counter.endLoadTime - counter.arrivalTime); // addition of all cycle
                                                                    // times..
        }

        return cycTime / cs.size(); // return average cycle time ( total time / number of
                                    // containers)
    }

    int nrContainers(List<ContainerMeasurement> cs) { // method to return number of containers
        return cs.size(); // returns size of list, which is the number of containers
    }

    int nrShips(List<ContainerMeasurement> cs) { // method to return number of ships
        return cs.get(cs.size() - 1).shipIdentifier; // returns shipIdentifier of the last
        // object in list
