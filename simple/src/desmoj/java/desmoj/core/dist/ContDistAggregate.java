...
/**
 * Aggregate Distribution composed of a list of input Distributions. The operator
 * between these to input distributions can be specified individually
 * 
...
public class ContDistAggregate extends ContDist {
     ...
     * is specified by a list of other distributions and an operator to aggregate them.<p>
     * 
     * Note that since it is possible to combine distributions of different
     * return types, this distribution will always cast their results to double
     * and return a double itself.
     * 
     * @param owner
     ...
     * @param dists
     *            List<NumericalDist> : The input distributions.
     * @param operator
    ...
    public ContDistAggregate(Model owner, String name, List<NumericalDist<?>> dists,
        ...
        if (operator == null) {
            ...
            sendWarning(
                    ...
                    "ContDistAggregate: " + this.getName() + ", constructor ContDistAggregate(Model owner, String name, List<NumericalDist<?>> dists, Operator operator, boolean showInReport, boolean showInTrace)",
        ...
        if (this.dists.isEmpty()) {
            ...
            sendWarning(
                    ...
                    "ContDistAggregate: " + this.getName() + ", constructor ContDistAggregate(Model owner, String name, NumericalDist<?> dist1, NumericalDist<?> dist2, Operator operator, boolean showInReport, boolean showInTrace)",
    ...
    /**
     * Creates a stream of pseudo random numbers following a distribution that
     * is specified by two other distributions and an operator to aggregate them.<p>
     * 
     * Note that since it is possible to combine distributions of different
     * return types, this distribution will always cast their results to double
     * and return a double itself.
     ...
     *            java.lang.String : The distribution's name
     * @param dist1
     *            NumericalDist<?> : The first input distribution.
     * @param dist2
     *            NumericalDist<?> : The second input distribution.
     * @param operator
     ...
     * before and the two input distributions as well as the operator specified
     * for this distribution.<p>
     * Evaluation is done from left to right, e.g. for three distributions, the
     * results is <code>(dist1.sample x d2.sample()) x d3.sample()</code>,
     * where <code>x</code> is the operation used.
     * 
    ...
    /**
     * Method to map a double <code>p</code> from 0...1 to the 
