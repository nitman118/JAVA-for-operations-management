...
public abstract class SimProcess extends Entity {
	 ...
	 * executing its <code>lifeCycle</code> method after the current
	 * <code>Schedulable</code> withdraws program control. Note, however,
	 * other <code>Schedulable</code>s may receive program control before
	 * the activated progess, compare remarks to method 
	 * <code>activate(TimeSpan dt)</code>.  
	 */
     ...
     * A <code>TimeInstant</code> representing the current time instant,
     * i.e. an argument equal to <code>presentTime()</code> can be used to 
     * <b>immediately</b> activate the process, implicitly stalling  
     * the current process until the activated process withdraws
     * program control, referred to as <b>process preemption</b>. If preemption
     * of the current process is not desired, use <code>activateAfter(this)</code> 
     * to schedule the activation of process directly <b>after</b> this current 
     * process (or other <code>Schedulable</code>) or 
     * <code>activate(new TimeSpan(0))</code> (or its shortcut <code>activate()</code>)
     * to activate a process directly after all other event list entries 
     * schedule to occur at the current instant in time with respect to 
	 ...
	 * is needed for scheduling here. <br/>
	 * The special argument <code>TimeSpan.ZERO</code> can be used to 
	 * <b>immediately</b> activate the process, implicitly stalling  
	 * the current process (if any) until the activated process withdraws
	 * program control, referred to as <b>process preemption</b>. If preemption
	 * of the current process is not desired, use <code>activateAfter(this)</code> 
	 * to schedule the activation of process directly <b>after</b> this current 
	 * process (or other <code>Schedulable</code>) or 
	 ...
	 * time. <br/>
	 * Passing the special argument <code>TimeSpan.ZERO</code> potentially causes 
	 * process preemption, i.e. temprorily stalls the current process (if any),
	 * compare method <code>activate(TimeSpan dt)</code>.
	 * 
	 * @param dt
