...
public class CanvasTimeSeries extends AbstractNumericalChartCanvas<Double> {
    ...
    private static Color getColor(int index) {
        ...
        switch (index) {
           case 0:
            ...
            case 11:
                color = new Color(120,150,210); break;
            case 12:
                color = new Color(106,50,154); break;
            case 13:
                color = new Color(94,55,41); break;
            case 14:                    
                color = new Color(104,0,31); break;
            case 15:                    
                color = new Color(180,144,90); break;
            case 16:
                color = new Color(120,88,54); break;
            case 17:
                color = new Color(238,180,159); break;
            case 18:
                color = new Color(184,135,100); break;
            case 19:
                color = new Color(43,23,71); break;
            case 20:
                color = new Color(140,152,248); break;
            case 21:
                color = new Color(255,255,128); break;
            case 22:
                color = new Color(70,10,20); break;
            case 23:
                color = new Color(42,79,140); break;
            default:
                color = Color.black; break;
        }
        return color;        
	...
	/**
	 * An List containing the color that represents each TimeSeries.
	 * The order of the entries in this List should be the same as those in _timeSeries.
	 */
	private List<Color> _dataColors;
	
	private static int nextColorAvailable = 0;
	
	public CanvasTimeSeries(String name, int canvasHeight, int canvasWidth, ChartDataTimeSeries timeSeriesData, String axis_x, String axis_y) {
	...
	void determineYScale() {
        ...
        for (candidate = 0; candidate < hight_candidates.length; candidate++) {
            if (Math.pow(10, power) * hight_candidates[candidate] > maxEntry) break;                                
        }
	...
	public Color getDataColor(int i) {
		if (i < 0 || i >= _timeSeries.size()) return Color.WHITE;
	...
	public long getNumOfYScale() {
		if (_numOfYScale == Long.MIN_VALUE) determineYScale();
		return _numOfYScale;
	}
	...
	public Double getYScale() {
		if (Double.isNaN(_yScale)) determineYScale();
		return _yScale;
	}
	...
	public long getNumOfXScale() {
		if (_numOfXScale == Long.MIN_VALUE) determineXScale();
		return _numOfXScale;
	}
	...
	public Double getXScale() {
		if (Double.isNaN(_xScale)) determineXScale();
		return _xScale;
	}
	...
	public Double getStartXScale() {
		if (Double.isNaN(_startXScale)) determineXScale();
		return _startXScale;
	}
	...
	public Double[] getDataValues(int i) {
    	if (i < 0 || i >= this.getNumOfTimeSeries()) return new Double[0];
    	else return _timeSeries.get(i).getDataValues();
	}
	...
	public Double[] getTimeValues(int i) {
    	if (i < 0 || i >= this.getNumOfTimeSeries()) return new Double[0];
    	else return _timeSeries.get(i).getTimeValues();
	}
