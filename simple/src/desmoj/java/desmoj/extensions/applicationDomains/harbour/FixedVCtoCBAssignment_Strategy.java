 ...
 * transporter (VC) gets the <code>Job</code> that has as origin or
 ...
 * Transporter, Queue Jobs) of the <code>TransportStategy</code>. The
 * FixedVCtoCBAssignment_Strategy is derived from ModelComponent.
 * 
...
public class FixedVCtoCBAssignment_Strategy extends ModelComponent implements
		TransportStrategy {
	/**
	 * the condition that checks the job for a VC has as the origin or
	 * destination the Crane which that VC is assigned to.
	 */
	private IsJobForTransporter cond;
	 ...
	 * every transporter (VC) gets the <code>Job</code> that has as origin or
	 * destination the crane that transporter is assigned to.
	 * 
	 * @param owner
	 *            desmoj.Model : The model this FixedVCtoCBAssignment_Strategy
	 *            is associated to.
	 */
	 ...
	 * transporters: every transporter (VC) gets the <code>Job</code> that has
	 * as origin or destination the crane that transporter is assigned to. This
	 * each assignment will be stored in a <code>TranspoterJob</code>. All
	 * the assignments are used by the transport control.
	 * 
	 * @param Jobs
	 *            <code>Queue</code>: The queue with the jobs of the
	 *            transporter control.
