...
<E extends EntityBasicAnimation> extends ModelComponent{
	...
	/**
	 * creates a route from sourceStation tp sinkStation. The
	 * route includes some intermediate points p[]
	 * @param owner
	...
	public TransportRouteAnimation(ModelAnimation owner, String name, double length, 
		...
		if(this.showInAnimation){
			try {
				if(init)	c = Command.getCommandInit("createRoute", this.cmdGen.getAnimationTime(simTime));
				else 		c = Command.getCommandTime("createRoute", this.cmdGen.getAnimationTime(simTime));
	...
	public boolean setAttributes(int show, int lineSize, Color color){
		...
		if(this.showInAnimation){
			try {
				if(init)	c = Command.getCommandInit("setRoute", this.cmdGen.getAnimationTime(simTime));
				else 		c = Command.getCommandTime("setRoute", this.cmdGen.getAnimationTime(simTime));
				c.addParameter("RouteId", this.id);
				c.addParameter("Show", Integer.toString(this.show));
				c.addParameter("LineSize", Integer.toString(this.lineSize));
				if(this.color != null){
	...
	/**
	 * Insert an entity at begin of route. The Entity moves on route 
	 * with velocity specified in entry attribute velocity.
	 * @param entity
	 * @return	true when successful
	 */
	...
	/**
	 * Insert an entity at begin of route. The velocity-attribute
	...
	public boolean insert(E entity, double velocity){
		...
		if(this.showInAnimation){
			try {
				...
				c.addParameter("Attribute", "velocity|"+Double.toString(velocity));
	...
	/**
	 * Insert an entity at begin of route. The entity passes route 
	 * in routeTime
	 * @param entity
	 * @param routeTime
	 * @return	true when successful
	...
	public boolean insert(E entity, TimeSpan routeTime){
		...
		double velocity = (double)(this.length / routeTime.getTimeRounded(TimeUnit.SECONDS));
