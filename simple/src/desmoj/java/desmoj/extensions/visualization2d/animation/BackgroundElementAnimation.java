 ...
 * - the other has a fixed middle point. The size is set also or comes from the text element.
 * It is possible to show a text in the rectangle. There is a automatic word wrapping. 
 * For an explicit newline use a <br> tag. The text- position, size and style can be set explicitly.
 * The colors of text and background can be set also. When background is null,
 * the background is transparent.
 * It is also possible to show an image on background. This image must loaded before in ModelAnimation. 
 * The level value determine the order of painting. Elements with low level value 
 * are painted in front of elements with high level.
 * 
...
public class BackgroundElementAnimation{
	 ...
	 * @param name				Can be null, when not null a border 
	 * 							with the name is shown.
	 * @param text				The text element, that is shown in background element,
	 * 							with automatic word wrapping. Use <br> for a new line.
	...
	public BackgroundElementAnimation(ModelAnimation owner, String baseId, String name, 
		...
		// Consistent checks
		if(topLeft == null) throw new SimulationException("topLeft is null");
		if(bottomRight == null) throw new SimulationException("bottomRight is null");
		if(this.text != null && this.textColor == null) throw new SimulationException("text has a value and foreground is null");
		...
		if(this.text.trim().equals("")) this.text = ".";

		if(this.showInAnimation){
			try {
				if(init)	c = Command.getCommandInit("createBackgroundElement", this.cmdGen.getAnimationTime(simTime));
				else 		c = Command.getCommandTime("createBackgroundElement", this.cmdGen.getAnimationTime(simTime));
				...
				if(bottomRight != null){
					String[] bottomRight1	= {bottomRight.getView(), Integer.toString(pBottomRight.x), Integer.toString(pBottomRight.y)};
	 ...
	 * @param form				Form of background element, 
	 * 							when null minimal size of text is used.
	 * @param fg				Text color
	 * @param bg				Background color, when null then transparent
	 * @param showInAnimation
	 */
	public BackgroundElementAnimation(ModelAnimation owner, String baseId, String name, 
		...
		// Consistent checks
		if(middle == null) throw new SimulationException("middle is null");
		if(this.text == null && form == null) throw new SimulationException("text and size are null");
		if(this.text != null && this.textColor == null) throw new SimulationException("text has a value and foreground is null");
		if(this.text == null || this.text.trim().equals("")) this.text = ".";
		...
		if(this.showInAnimation){
			try {
				...
				if(middle != null){
					String[] middle1		= {middle.getView(), Integer.toString(middle.getPoint().x), Integer.toString(middle.getPoint().y)};
	 ...
	 * The imageId must defined before in ModelAnimation.addIcon(..)
	 * @param imageId
	 */
	...
	private void setData(){
		...
		if(this.showInAnimation){
			...
			Command c;
			try {
				if(init)	c = Command.getCommandInit("setBackgroundElement", this.cmdGen.getAnimationTime(simTime));
				else 		c = Command.getCommandTime("setBackgroundElement", this.cmdGen.getAnimationTime(simTime));
