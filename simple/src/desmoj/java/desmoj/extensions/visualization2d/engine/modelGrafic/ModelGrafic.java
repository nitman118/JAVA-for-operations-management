...
public class ModelGrafic extends JPanel implements Grafic, ChangeListener {
	...
	public void reset(){
		...
		if(this.viewList.containsKey(codeFirst))this.addToTabbedPane(codeFirst);
		...
		for(Enumeration<String> en=this.viewList.keys(); en.hasMoreElements(); ){
			...
			if(! code.equals(codeFirst)) idsorted.add(code);
		}
	...
	public Rectangle getBoundsExtern(){
		...
		Rectangle out = null;
		if(vg != null) out = vg.getBoundsExtern();
		return out;
	}
	...
	public boolean makeSnapShot(File directory, boolean all) throws IOException{
		...
		if( this.viewList == null || this.viewList.isEmpty()) return false;
		String timeStr = SimulationTime.getTimeString(model.getSimulationTime().getSimulationTime(),SimulationTime.SHOW_DAY_DATE_TIME_MILLIS_DST);
	...
	public class ZoomEntry{
		...
		public ZoomEntry(String viewId, boolean isSelected, double zoomFactor, Point zoomPoint, Rectangle viewRectangle){
			this.viewId			= viewId;
