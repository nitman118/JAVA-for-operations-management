...
/**
 * Captures all relevant information about the <code>TransportJunction</code>.
 * That means it collects all data from the master (transporter) and the slave
 * (goods) queue.
 * 
...
public class TransportReporter extends desmoj.core.report.Reporter {
	...
	/**
	 * Constructor for a new TransportReporter. Note that although any
	 * Reportable is accepted you should make sure that only subtypes of
	 * TransportJunction are passed to this constructor. Otherwise the number of
	 * column titles and their individual headings will differ from the actual
	 * content collected by this reporter.
	 * 
	 * @param informationSource
	 *            desmoj.Reportable : The TransportJunction to report about
	 */
	 ...
	 * fetching the data from the master (transporter) and slave (goods) queue.
	 * Implement this method in a way, that an array of the same length as the
	 * columntitles is produced containing the data at the point of time this
	...
	public String[] getEntries() {
		...
		if (source instanceof desmoj.extensions.applicationDomains.production.TransportJunction) {
			...
			// informationsource)
			desmoj.extensions.applicationDomains.production.TransportJunction tj = (desmoj.extensions.applicationDomains.production.TransportJunction) source;
