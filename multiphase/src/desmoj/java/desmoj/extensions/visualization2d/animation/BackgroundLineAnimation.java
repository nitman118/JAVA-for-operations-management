...
public class BackgroundLineAnimation{
	...
	public BackgroundLineAnimation(ModelAnimation owner, String baseId, int lineSize, 
		...
		if(this.showInAnimation){
			try {
				if(init)	c = Command.getCommandInit("createBackgroundLine", this.cmdGen.getAnimationTime(simTime));
				else 		c = Command.getCommandTime("createBackgroundLine", this.cmdGen.getAnimationTime(simTime));
				c.addParameter("BgLineId", id);
				c.addParameter("LineSize", Integer.toString(this.lineSize));
				c.addParameter("StartPoint", this.startPoint.getView()+"|"+this.startPoint.getPoint().x+"|"+this.startPoint.getPoint().y);
