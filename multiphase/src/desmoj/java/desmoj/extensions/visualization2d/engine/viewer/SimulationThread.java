...
public class SimulationThread extends Thread {
		...
		while(!this.isInterrupted() && s1 < Long.MAX_VALUE){
			...
			if(this.isWorking()){
				...
				case ViewerPanel.TimeFlowMode_CONTINIUM:
					...
					if (debugOutput) System.out.println("continuous SimTime from: "+s0+" until: "+s1+
				...
				case ViewerPanel.TimeFlowMode_STEP_FLOW:
					...
					this.viewer.getSimulationTimeInstance().setSimulationTime(s1);
					this.viewer.updateSimulationTime(true);
					if (debugOutput) System.out.println("stepFlow SimTime from: "+s0+" until: "+s1+
				...
				case ViewerPanel.TimeFlowMode_STEP_SINGLE:
					...
					this.viewer.getSimulationTimeInstance().setSimulationTime(s1);
					this.viewer.updateSimulationTime(true);
					if (debugOutput)  System.out.println("single Step SimTime: "+s0+" nextCmd: "+s1+
					  "  ViewerZoom: "+viewer.getSimulationZoom()+"  ViewerSpeed: "+speed );
					break;
				}
				// update
	...
	/**
	 * Make a delay of delay millisec. and shows simulation time in viewer from s0 until s1.
	 * The delay is finished when isWorking switch to false.
	 * Every 100 millisec the simulation time is updated.
