...
/**
 * CommandFrame includes basic-methods for command-processing.
 * In CommandSyntax describes the syntax of all commands-types.
 * Command is the only subclass of CommandFrame. The method Command.execute 
 * makes a semantic check and execute a command.
 * 
 * For every command-type, commands with the same command-parameter-value, 
 * described in CommandSyntax, exists a template, stored in 
 * a hashtable under it's name. When a command of this type is created, a clone 
 * of the assizioated command-type is generated and filed with the parameters.
 * 
...
abstract class CommandFrame implements CommandSyntax, Cloneable{
	...
	static{
		try{CommandFrame.init_cmdTemplate();
	...
	public static void init_cmdTemplate() throws CommandException{
		...
		for(int i=0; i<CMD_SYNTAX.length; i++){
			...
			for(int j=0; j<anzParameter; j++){
				...
				// + heisst 1 .. unendlich
				if(quantifier.equals("+")) {parameter[j] = new Parameter(type, 1, Integer.MAX_VALUE); }
				
				// * heisst 0 .. unendlich
				else if(quantifier.equals("*")) {parameter[j] = new Parameter(type, 0, Integer.MAX_VALUE); }
					...
					try{
						...
						if(quantifier.length() > k+2){
							...
							if(0 <= min  && min <= max){
								...
								throw new CommandException("CommandFrame.init_cmdTemplate : Quantifier is not valid", "cmd: "+cmd+" quantifier: "+quantifier+" type: "+type);
							}
						}else{
							throw new CommandException("CommandFrame.init_cmdTemplate : Quantifier is not valid", "cmd: "+cmd+" quantifier: "+quantifier+" type: "+type);
						}
				...
				// + heisst feste Anzahl
					...
					try{
						...
						if(q >= 0) parameter[j] = new Parameter(type, q, q);
						else{
							parameter[j] = null;
							throw new CommandException("CommandFrame.init_cmdTemplate : Quantifier ist negativ", "cmd: "+cmd+" quantifier: "+quantifier+" type: "+type);
						}
					}catch (NumberFormatException e){
						throw new CommandException("CommandFrame.init_cmdTemplate: Quantifier ist kein int", "cmd: "+cmd+" quantifier: "+quantifier+" type: "+type);
					}
				}
			}
	...
	public static CommandFrame getCommandInit(String cmd, long initTime)throws CommandException{
		...
		if(command == null)throw new CommandException("CommandFrame.getTemplateInit: Command is not in CommandFrame.cmdTemplate", "cmd: "+cmd);
		try{
			out = (CommandFrame)command.clone();
	...
	public static CommandFrame getCommandTime(String cmd, long time)throws CommandException{
		CommandFrame out = null;
		CommandFrame command = cmdTemplate.get(cmd);
		if(command == null)throw new CommandException("CommandFrame.getTemplateTime: Command is not in CommandFrame.cmdTemplate", "cmd: "+cmd);
		try{
			out = (CommandFrame)command.clone();
			out.setTime(time);
			out.setNoTemplate();
	...
	public static CommandFrame parseCommand(String cmd)throws CommandException{
		...
		do{
			...
			if(tokenPos != -1){
				...
				throw new CommandException("CommandFrame.parseCommand Parameter \""+part+"\" of Command include no \""+Cmd.KEY_VALUE_SEPARATOR+"\" Separator", cmd);
			}
		}while(! cmdTmp.equals(""));
		//System.out.println("Command.parseCommand end");
		if(command == null) System.out.println("cmd it null "+cmd);
		return command;
	}
	...
	public Parameter getParameter(int i){
		Parameter out = null;
		if(i < this.parameter.length) out = this.parameter[i];
		return out;
	}
	...
	public void addParameter(String key, String value) throws CommandException{
		...
		if(p == null) throw new CommandException("CommandFrame.addParameter key: "+key+" is not valid.", this.toString());
		p.addValue(value);
	}
	 ...
	 * The component of the value are splited in an array
	 * @param key
	 * @param i
		...
		if(! CommandFrame.existCommand(this.cmd)){
			...
			for(int i=0; i<this.parameter.length; i++){
				...
				if(! (p.getMin()<= p.getValues().length)){
					out = false;
					throw new CommandException("CommandFrame.syntaxCheck Not enough parameter of type: "+p.getTyp(), this.toString());
				}
				if(! (p.getValues().length <= p.getMax())){
					out = false;
					throw new CommandException("CommandFrame.syntaxCheck To much parameter of type: "+p.getTyp(), this.toString());
				}
			}
		}
		return out;
	...
	protected void write(PrintWriter f){
		//public String write(){
			...
			if(this.isInit()){
				...
				p	=	Cmd.INIT_KEY+Cmd.KEY_VALUE_SEPARATOR+Boolean.toString(true)+Cmd.PARAMETER_SEPARATOR;
				out	+=	String.format(initForm, p);
			}else{
				p	=	Cmd.TIME_KEY+Cmd.KEY_VALUE_SEPARATOR+this.getTime()+Cmd.PARAMETER_SEPARATOR;
				out	+=	String.format(timeForm, p);
				p	=	Cmd.INIT_KEY+Cmd.KEY_VALUE_SEPARATOR+Boolean.toString(false)+Cmd.PARAMETER_SEPARATOR;
				out	+=	String.format(initForm, p);
			}
			// write cmd
			...
			for(int j=0; this.getParameter(j)!= null; j++){
				...
				if(parameter != null){
					for(int i=0; i<parameter.getValues().length; i++){
						p	= parameter.getTyp()+Cmd.KEY_VALUE_SEPARATOR+parameter.getValues()[i]+Cmd.PARAMETER_SEPARATOR;
