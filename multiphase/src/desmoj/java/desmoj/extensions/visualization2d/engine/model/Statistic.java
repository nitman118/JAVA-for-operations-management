 ...
 * In a time series, for every observed time, you have exactly one value.
 * In a observation series, you can have more than one value at one time.
 * E.g. At one time, you have 2 events with different values.
 * On the fly, the indexes: INDEX_None, INDEX_Min_Max, INDEX_Mean_StdDev
 * are supported.
 * For visualization you must use the class StatisticGrafic.
 * The data are stored in data structures of the jFreeChart Library.
 * For histogram support the number of histogram-cells specified by 
 * constructor must greater than 0.
 * 
...
public class Statistic implements Basic{
	 ...
	 * @param typeData			one of the following values:
	 * 							Statistic.DATA_Observations
	 * 							Statistic.DATA_TimeSeries
	 * @param typeIndex			one of the following values:
	 * 							Statistic.INDEX_None
	 * 							Statistic.INDEX_Min_Max
	 * 							Statistic.INDEX_Mean_StdDev
	 * @param aggregate			true:  the values will be aggregated
	 * 							false: the original values will be used
	 * @param timeLow			lowest expected time, will be changed when a lower time is observed.
	 * 							Used to avoid axis changes.
	 * @param timeHigh			highest expected time, will be changed when a higher time is observed.
	 * 							Used to avoid axis changes.
	 * @param valueLow			lowest expected value, will be changed when a lower value is observed.
	 * 							Used to avoid axis changes.
	 * @param valueHigh			highest expected value, will be changed when a higher value is observed.
	 * 							Used to avoid axis changes.
	 * @param histogramCells	No. of histogram cells between valueLow and valueHigh.
	 * 							Every cell has the same length.
	 * 							For values outside of [valueLow, valueHigh] additional cells will be created.
	 * 							When <= 0, no histogram data support.
	 */
	public Statistic(Model model, String id, int typeData, int typeIndex, boolean aggregate, 
		...
		if(this.id != null) model.getStatistics().add(this);
	...
	public void reset(){
		...
		if(this.grafic_LastValue!= null)((StatisticGrafic)this.grafic_LastValue).update();
		if(this.grafic_Histogram!= null)((StatisticGrafic)this.grafic_Histogram).update();
		if(this.grafic_TimeValueDiagram!= null)((StatisticGrafic)this.grafic_TimeValueDiagram).update();
		//if(this.getGrafic()!= null)((StatisticGrafic)this.getGrafic()).update();
	}

	
	/**
	 * init / reset Statistic Object
	 * called by Constructor and reset
	...
	private void init(boolean init){
		...
		case Statistic.DATA_Observations:
			if(init)	this.observationSerie			= new XYSeries(this.id);
			else		this.observationSerie.clear();
			break;
		case Statistic.DATA_TimeSeries:
			if(init)	this.timeSerie					= new TimeSeries(this.id);
			else		this.timeSerie.clear();
			break;
		}
	...
	public void setName(String name){
		this.name	= name;
		if(this.histogram != null) 			this.histogram.setKey(name);
		if(this.observationSerie != null) 	this.observationSerie.setKey(name);
		if(this.timeSerie != null) 			this.timeSerie.setKey(name);
	}
	
	/**
	 * add a new point to the time/observation series.
	 * For time series the times must strictly increasing.
	 * For observation series the times must (not strictly)increasing.
	 * @param value
	 * @param time
	...
	public void update(double value, long time) throws ModelException{
		...
		case Statistic.DATA_Observations:
			...
			break;
		case Statistic.DATA_TimeSeries:
			if(observationCount >0 && lastTime == time) vat.addValue(value);
			else vat = new ValuesAtTime(time, value);
		...
		if(value < this.valueLow) 	this.valueLow 	= value;
		if(value > this.valueHigh) 	this.valueHigh 	= value;
		if(time  < this.timeLow) 	this.timeLow 	= time;
		if(time  > this.timeHigh) 	this.timeHigh 	= time;
	...
	/**
	 * gets TypeData. Possible values: 
	 * Statistic.DATA_Observations or Statistic.DATA_TimeSeries
	 * @return
	 */
	...
	/**
	 * gets TypeData. Possible values: 
	 * Statistic.INDEX_None, Statistic.INDEX_Min_Max or 
	 * Statistic.INDEX_Mean_StdDev
	 * @return
	 */
	...
	public double getLastValue() throws ModelException{
		if(!(this.hasValue()))throw new ModelException("No Observations stored.");
	 ...
	 * for typeData = Statistic.DATA_TimeSeries, the time depended mean is computed.
	 * @return
	 ...
	 * for typeData = Statistic.DATA_Observations, all values have the same weight.
	 * for typeData = Statistic.DATA_TimeSeries, the time depended standard deviation is computed.
	 * @return
	 * @throws ModelException, when typeIndex != Statistic.INDEX_Mean_StdDev
	 */
	 ...
	 * @param animationType     Possible values:
	 ...
	 * @param deltaSize			Each animation type has a default size. 
	 * 							This size can be incremented/decremented by deltaSize. 
	 * 							Null means no change.
	 * @return
	 * @throws ModelException
	 */
	public Grafic createGrafic(String viewId, int x, int y, int animationType, boolean isIntValue, Dimension deltaSize, boolean infopane)throws ModelException{
		Grafic out = null;
		...
		case(StatisticGrafic.ANIMATION_LastValue):
			...
			if(this.grafic == null) this.grafic = this.grafic_LastValue;
		...
		case(StatisticGrafic.ANIMATION_Histogram):
			this.grafic_Histogram = new StatisticGrafic(this, viewId, new Point(x,y),  
					animationType, isIntValue, deltaSize, infopane);
			if(this.grafic == null) this.grafic = this.grafic_Histogram;
		...
		case(StatisticGrafic.ANIMATION_TimeValueDiagram):
			this.grafic_TimeValueDiagram = new StatisticGrafic(this, viewId, new Point(x,y),  
					animationType, isIntValue, deltaSize, infopane);
			if(this.grafic == null) this.grafic = this.grafic_TimeValueDiagram;
	 ...
	 */
	public desmoj.extensions.visualization2d.engine.modelGrafic.Grafic getGrafic(){
		return this.grafic;
	}
