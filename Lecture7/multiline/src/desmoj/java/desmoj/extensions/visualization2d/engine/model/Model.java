 ...
 * the animation-objects are accessable over a static animation.orga.ClassContent hashtable.
 *  
...
public class Model {
	 ...
	 */
	public Model(URL simulationIconDir, CoordinatenListener coordinatenListener, ViewerPanel viewer)throws ModelException{
	...
	public ModelGrafic getModelGrafic()throws ModelGraficException{
		if(this.modelGrafic == null) System.out.println("Model.getModelGrafic() is null");
		return this.modelGrafic;
	}
	...
	public void createModelBasisData(String[] projectName, String[] projectURL, 
		...
		if(projectName.length > 0) this.modelProjectName = projectName[0];

		this.modelProjectURL	= null;
		if(projectURL.length > 0) this.modelProjectURL = projectURL[0];
		
		this.modelProjectIconId= null;
		if(projectIconId.length > 0) this.modelProjectIconId = projectIconId[0];

		this.modelName			= modelName;
		this.modelAuthor		= modelAuthor;
		
		this.modelCreatedAt		= "";
		if(modelDate.length > 0) this.modelCreatedAt = modelDate[0];
		
		this.modelDescription	= "";
		if(modelDescription.length > 0) this.modelDescription = modelDescription[0];
		...
		this.modelLicence		= "";
		if(modelLicense.length > 0) this.modelLicence = modelLicense[0];

		this.desmojVersion	= "";
		if(desmojVersion.length > 0) this.desmojVersion = desmojVersion[0];
			
		this.desmojLicense	= "";
		if(desmojLicense.length > 0) this.desmojLicense = desmojLicense[0];
			
		this.desmojLicenseURL	= null;
		if(desmojLicenseURL.length > 0) this.desmojLicenseURL = desmojLicenseURL[0];
	}
	 ...
	 * @throws ModelException
	 */
	public void createSimulationTimeBounds(String begin, String[] endArray, String[] timeZone, String defaultSpeed, boolean isInit, long time)throws ModelException{
		try{this.simulationBegin	= Long.parseLong(begin);
		}catch(java.lang.NumberFormatException e){
			throw new ModelException("Model.createSimulationTimeBounds: SimulationBegin is no long: "+begin);
		}
		if(endArray.length >= 1){
			try{this.simulationEnd		= Long.parseLong(endArray[0]);
			}catch(java.lang.NumberFormatException e){
		...
		try{this.simulationSpeed	= Double.parseDouble(defaultSpeed);
		}catch(java.lang.NumberFormatException e){
			throw new ModelException("Model.createSimulationTimeBounds: SimulationDefaultSpeed is no double: "+defaultSpeed);
		}
	}
	...
	public void createImage(String id, String datei, boolean isInit, long time)throws ModelException{
		...
		if(ii.getImageLoadStatus() == MediaTracker.ERRORED)
			throw new ModelException("Error on loading ImageIcon URL: "+imageURL.toString());
	...
	public void createEntityTyp(String id, String width, String height, 
			String[] posStates,	String[] posAttributes, String show, boolean isInit, long time) throws ModelException{
		...
		try{
			...
			throw new ModelException("Model.createEntityType: iconSize is no integer pair: ("+width+", "+height+")");
		}
		...
		for(int i=0; i<posStates.length; i++){
			...
			if(split.length == 2) posStates1[i] = new Attribute(split[0], split[1]);
			else throw new ModelException("Model.createEntityTyp The posibleStates attribute must have 2 parts, state and imageId. attribute:"+posStates[i]);
		}
	...
	public void createBackgroundElement(String id, String[] name, String[] text, 
		...
		if(imageId.length > 0) imageId1 = imageId[0];
		...
		if(text.length > 0){
			...
			if(tmp.length != 4) throw new ModelException("text parameter hasn't 4 parts! Text parameter: "+text[0]);
			try{
				textStr	= tmp[0];
		...
		if(topLeft.length > 0){
			...
			if(tmp.length != 3)
				throw new ModelException("Model.createBackgroundElement: topLeft parameter has wrong form: "+topLeft[0]+"  id: "+id);
			try{
				viewId		= tmp[0];
		...
		if(bottomRight.length > 0){
			...
			if(tmp.length != 3)
				throw new ModelException("Model.createBackgroundElement: bottomRight parameter has wrong form: "+bottomRight[0]+"  id: "+id);
			try{
				viewId			= tmp[0];
		...
		if(middle.length > 0){
			...
			if(tmp.length != 3)
				throw new ModelException("Model.createBackgroundElement: middle parameter has wrong form: "+middle[0]+"  id: "+id);
			try{
				viewId	= tmp[0];
		...
		if(size.length > 0){
			...
			if(tmp.length != 2)
				throw new ModelException("Model.createBackgroundElement: size parameter has wrong form: "+size[0]+"  id: "+id);
			try{
		...
		if(foreground.length > 0){
			...
			if(tmp.length != 3)
				throw new ModelException("Model.createBackgroundElement: foreground parameter has wrong form: "+foreground[0]+"  id: "+id);
			try{
		...
		if(background.length > 0){
			...
			if(tmp.length != 3)
				throw new ModelException("Model.createBackgroundElement: background parameter has wrong form: "+background[0]+"  id: "+id);
			try{
		...
		backgroundElement = new BackgroundElement(this, id, textStr, textLocation, textSize, textStyle, level1, imageId1);
		if(name.length > 0) backgroundElement.setName(name[0]);
		if(middle1 == null){
	...
	public void setBackgroundElement(String id, String[] name, String[] text, String[] foreground, String[] background, String[] imageId, boolean isInit, long time){
		...
		if(isInit)	time = this.getSimulationBegin();

		BackgroundElement backgroundElement = this.getBackgroundElements().get(id); 
		if(backgroundElement != null){
			if(text.length > 0){
				tmp = Parameter.split(text[0]);
				if(tmp.length != 4) throw new ModelException("Model.setBackgroundElement: text parameter hasn't 4 parts! Text parameter: "+text[0]);
				try{
					textStr	= tmp[0];
					textLocation 	= Integer.parseInt(tmp[1]);
					textSize 		= Integer.parseInt(tmp[2]);
				...
				if(foreground.length > 0){
					tmp = Parameter.split(foreground[0]);
					if(tmp.length != 3)
						throw new ModelException("Model.setBackgroundElement: foreground parameter has wrong form: "+foreground[0]+"  id: "+id);
					try{
						foreground1	= new Color(Integer.parseInt(tmp[0]),Integer.parseInt(tmp[1]),Integer.parseInt(tmp[2]));
					}catch(NumberFormatException e){
						throw new ModelException("Model.setBackgroundElement: foreground parameter has wrong form: "+foreground[0]+"  id: "+id);
				...
				if(background.length > 0){
					tmp = Parameter.split(background[0]);
					if(tmp.length != 3)
						throw new ModelException("Model.setBackgroundElement: background parameter has wrong form: "+background[0]+"  id: "+id);
					try{
						background1	= new Color(Integer.parseInt(tmp[0]),Integer.parseInt(tmp[1]),Integer.parseInt(tmp[2]));
					}catch(NumberFormatException e){
						throw new ModelException("Model.setBackgroundElement: background parameter has wrong form: "+background[0]+"  id: "+id);
			...
			if(name.length > 0) backgroundElement.setName(name[0]);
			if(text.length > 0) {backgroundElement.setData(textStr, textLocation, textSize, textStyle, foreground1, background1, imageId1);
			}
		}else{
			throw new ModelException("Model.setBackgroundElement BackgroundElementId does not exist. BackgroundElementId: "+id);
		}
	}
	
	public void createBackgroundLine(String id, String[] lineSize, String[] color, String startPoint, String[] addPoint, String level, boolean isInit, long time){
		...
		if(color.length > 0){
			...
			if(tmp.length != 3)
				throw new ModelException("Model.createBackgroundLine: color parameter has wrong form: "+color[0]+"  id: "+id);
			try{
				color1	= new Color(Integer.parseInt(tmp[0]),Integer.parseInt(tmp[1]),Integer.parseInt(tmp[2]));
			}catch(NumberFormatException e){
				throw new ModelException("Model.createBackgroundLine: foreground parameter has wrong form: "+color[0]+"  id: "+id);
			}
		}
		...
		if(startPoint != null){
			...
			try{
				...
				throw new ModelException("Model.createBackgroundLine Id: "+id+"  parse Error startPoint: "+startPoint);
			}
		}else{
		...
		for(int i=0; i<addPoint.length; i++){
			...
			try{
				...
				throw new ModelException("Model.createBackgroundLine Id: "+id+"  parse Error addPoint: "+addPoint[i]);
			}
		}
	...
	public void createEntity(String id, String[] name, String entityTypeId, 
			String[] velocity,	String state, String[] attribute, String[] position, boolean isInit, long time) throws ModelException{
		...
		if(position.length > 0){
			try{
				...
				throw new ModelException("Model.createEntity  EntityId: "+id+"  parse Error position: "+position[0]);
			}
		}
		...
		if(name.length > 0) e.setNameAttribute(name[0], time);
		if(velocity.length > 0) e.setVelocityAttribute(velocity[0], time);
		if(position.length == 0) e.createGraficFree(time);
		else	e.createGraficStatic(viewId, x, y, an, dir, time);
		...
		for(int i=0; i<attribute.length; i++){
			...
			if(entityType.existPossibleAttribut(tmp[0])){
				...
				throw new ModelException("Model.setEntity  EntityId: "+id+"  Attribute "+tmp[0]+" is not possible.");
			}
		}
	}
	 ...
	 * @throws ModelException
	 */
	public void setEntity(String id, String[] velocity, String[] state, String[] attribute, boolean isInit, long time) throws ModelException{
		if(isInit)	time = this.getSimulationBegin();
		...
		if(entity != null){
			...
			String[] tmp;
			if(velocity.length > 0)	entity.setVelocityAttribute(velocity[0], time);
			if(state.length > 0) 	entity.setState(state[0], time);
			for(int i=0; i<attribute.length; i++){
				tmp	= Parameter.split(attribute[i]);
				if(entityType.existPossibleAttribut(tmp[0])){
	...
	public void createList(String id, String[] name, String defaultEntityType, 
			String numberOfVisible,	String form, String point, String[] deltaSize, String[] comment, boolean isInit, long time) throws ModelException{
		...
		try{
			...
			throw new ModelException("Model.createList  ListId: "+id+"  parse Error numberOfVisible: "+numberOfVisible);
		}
		
		if(form.trim().equals("horizontal")) 	horizontal = true;
		else if(form.trim().equals("vertikal")) horizontal = false;
		...
		tmp[0] = "0"; tmp[1] = "0";
		if(deltaSize.length>0) tmp = Parameter.split(deltaSize[0]);
		try{
		...
		if(! this.getEntityTyps().exist(defaultEntityType)){
			throw new ModelException("Model.createList  ListId: "+id+"  DefaultEntityType does not exist: "+defaultEntityType);
		}
		...
		if(name.length > 0)	list.setName(name[0]);
		...
		if(comment.length > 0){
			try{
				...
				list.setCommentColor(new Color(Integer.parseInt(tmp[3]), Integer.parseInt(tmp[4]), Integer.parseInt(tmp[5])));
	 ...
	 * @throws ModelException
	 */
	public void setList(String id, String[] addEntity, String[] addEntityAfter, String[] addEntityBefore, String[] removeEntity, String[] removeAll, boolean isInit, long time) throws ModelException{
		if(isInit)	time = this.getSimulationBegin();
		...
		if(removeAll.length > 0){
			...
			for(int i=0; i<ids.length; i++) list.removeFromContainer(ids[i], time);
		}
		// add Entity
		for(int i=0; i<addEntity.length; i++){
			...
			if(tmp.length == 3){
				if(list.containsInContainer(tmp[0])){
					throw new ModelException("Model.setList addEntity Entity is alredy in list. ListId: "+id+"  Entity: "+tmp[0]);
				}
				try{
					prio = Integer.parseInt(tmp[1]);
				}catch(java.lang.NumberFormatException e){
					throw new ModelException("Model.setList addEntity prio is no Integer. ListId: "+id+"   prio: "+tmp[1]);
				}
		...
		for(int i=0; i<addEntityAfter.length; i++){
			...
			if(tmp.length == 3){
				if(list.containsInContainer(tmp[0])){
					throw new ModelException("Model.setList addEntityAfter Entity is alredy in list. ListId: "+id+"  Entity: "+tmp[0]);
				}
				try{
					prio = Integer.parseInt(tmp[1]);
				}catch(java.lang.NumberFormatException e){
					throw new ModelException("Model.setList addEntityAfter prio is no Integer. ListId: "+id+"   prio: "+tmp[1]);
				}
				if(! list.containsInContainer(tmp[2])){
					throw new ModelException("Model.setList addEntityAfter EntityAfter is not in list. ListId: "+id+"  EntityAfter: "+tmp[0]);
				}
		...
		for(int i=0; i<addEntityBefore.length; i++){
			...
			if(tmp.length == 3){
				if(list.containsInContainer(tmp[0])){
					throw new ModelException("Model.setList addEntityBefore Entity is alredy in list. ListId: "+id+"  Entity: "+tmp[0]);
				}
				try{
					prio = Integer.parseInt(tmp[1]);
				}catch(java.lang.NumberFormatException e){
					throw new ModelException("Model.setList addEntityBefore prio is no Integer. ListId: "+id+"   prio: "+tmp[1]);
				}
				if(! list.containsInContainer(tmp[2])){
					throw new ModelException("Model.setList addEntityBefore EntityBefore is not in list. ListId: "+id+"  EntityBefore: "+tmp[0]);
				}
	...
	public void createStation(String id, String[] name, String point, boolean isInit, long time) throws ModelException{
		...
		if(name.length > 0)	station.setName(name[0]);
	...
	public void createRoute(String id, String[] name, String length, 
			String sourceStationId,	String sinkStationId, String[] point, String show, String[] color, String lineSize, boolean isInit, long time) throws ModelException{
		...
		if(! this.getStations().exist(sourceStationId)){
			throw new ModelException("Model.createRoute  RouteId: "+id+"  SourceStation does not exist: "+sourceStationId);
		}
		if(! this.getStations().exist(sinkStationId)){
			throw new ModelException("Model.createRoute  RouteId: "+id+"  SinkStation does not exist: "+sinkStationId);
		}
		...
		if(color.length > 0){
			...
			try{
				...
				throw new ModelException("Model.createRoute: foreground parameter has wrong form: "+color[0]+"  id: "+id);
			}
		}
		...
		if(name.length > 0)	route.setName(name[0]);
	...
	public void setRoute(String id, String[] addEntity, String[] removeEntity, 
		...
		if(removeAll.length > 0){
			...
			for(int i=0; i<ids.length; i++) route.removeFromContainer(ids[i], time);
		}
		...
		if(color.length > 0){
			...
			try{
				...
				throw new ModelException("Model.setRoute: foreground parameter has wrong form: "+color[0]+"  id: "+id);
			}
			route.setColor(color1);
		}
	...
	public void createProcess(String id, String[] name, String numberOfResEntity, String numberOfProcEntity,	
			String defaultResType, String defaultProcType,	String[] listId, String form, String point,  boolean isInit, long time) throws ModelException{
		...
		try{
			...
			throw new ModelException("Model.createProcess  ProcessId: "+id+"  parse Error numberOfResEntity: "+numberOfResEntity);
		}
		try{
			anzProcEntity	= Integer.parseInt(numberOfProcEntity);
		}catch(java.lang.NumberFormatException e){
			throw new ModelException("Model.createProcess  ProcessId: "+id+"  parse Error numberOfProcEntity: "+numberOfProcEntity);
		}
		tmp = Parameter.split(point);
		try{
			viewId	= tmp[0];
		...
		if(! this.getEntityTyps().exist(defaultResType)){
			throw new ModelException("Model.createProcess  ProcessId: "+id+"  DefaultResEntityType does not exist: "+defaultResType);
		}
		if(! this.getEntityTyps().exist(defaultProcType)){
			throw new ModelException("Model.createProcess  ProcessId: "+id+"  DefaultProcEntityType does not exist: "+defaultProcType);
		}
		...
		if(listId.length > 0)	listValue	= List.PREFIX_QUEUE+listId[0];
		else					listValue	= null;
		
		Process process = new Process(this, id, anzResEntity, anzProcEntity, listValue);
		if(name.length > 0)	process.setName(name[0]);
	...
	public void setProcess(String id, String[] addResEntity, String[] removeResEntity, 
			String[] addProcEntity, String[] removeProcEntity,  boolean isInit, long time) throws ModelException{
		if(isInit)	time = this.getSimulationBegin();
	...
	public void createProcessNew(String id, String[] name, String[] abstractProc, 
			...
			String form, String[] showResources, String[] deltaSize,  String[] comment, boolean isInit, long time) throws ModelException{
		...
		if(! this.getEntityTyps().exist(defaultEntityType)){
			throw new ModelException("Model.createProcessNew  ProcessId: "+id+"  DefaultEntityType does not exist: "+defaultEntityType);
		}

		try{ anzVisibleValue = Integer.parseInt(anzVisible);
		}catch(NumberFormatException e){
		...
		try{
			...
			throw new ModelException("Model.createProcessNew  ProcessId: "+id+"  parse Error deltaSize: "+deltaSize);
		}
		...
		if(abstractProc.length == 0){
			...
			if(resourceType.length == 0)
				throw new ModelException("Model.createProcessNew  It's a abstact Process. Id: "+id+"  There is no resourceType");
			if(resourceTotal.length == 0)
				throw new ModelException("Model.createProcessNew  It's a abstact Process. Id: "+id+"  There is no resourceTotal");
			try{ resourceTotalValue = Integer.parseInt(resourceTotal[0]);
			}catch(NumberFormatException e){
				throw new ModelException("Model.createProcessNew  ResourceTotal is't a integer.");
			}
			process = new ProcessNew(this, ProcessNew.PREFIX_PROCESS, id, resourceType[0], resourceTotalValue, listIdValue);
		}
		if(name.length > 0)	process.setName(name[0]);
		...
		if(comment.length > 0){
			try{
				...
				process.setCommentColor(new Color(Integer.parseInt(tmp[3]), Integer.parseInt(tmp[4]), Integer.parseInt(tmp[5])));
				process.setCommentSizeExt(Boolean.parseBoolean(tmp[6]));
			}catch(java.lang.NumberFormatException e){
				throw new ModelException("Model.createProcessNew  ProcessId: "+id+"  parse Error Comment: "+comment);
			}
		}
	...
	public void setProcessNew(String id, String[] addProcEntity, String[] addResEntity, 
			String[] addResAnz, String[] removeEntity,  boolean isInit, long time) throws ModelException{
		...
		for(int i=0; i<addProcEntity.length; i++){
			...
			for(int j=0; j<ids.length; j++){
				if(! this.getEntities().exist(ids[j]))
					throw new ModelException("Model.setProcessNew - addProcEntity EntityId does not exist. EntityId: "+ids[j]);
				}
		}
		...
		for(int i=0; i<addResEntity.length; i++){
			...
			for(int j=0; j<ids.length; j++){
			if(! this.getEntities().exist(ids[j]))
				throw new ModelException("Model.setProcessNew - addResEntity EntityId does not exist. EntityId: "+ids[j]);
			}
		}
		
		for(int i=0; i<removeEntity.length; i++){
			if(! this.getEntities().exist(removeEntity[i]))
				throw new ModelException("Model.setProcessNew - removeEntity EntityId does not exist. EntityId: "+removeEntity[i]);
		}
		...
		if(process.isAbstractResource() && addProcEntity.length > 0){
			...
			for(int i=0; i<addResAnz.length; i++){
				try{ addResAnzValue[i] = Integer.parseInt(addResAnz[i]);
				}catch(NumberFormatException e){
	...
	public void createResource(String id, String[] name, String[] resourceType, 
			...
			String anzVisible, String form, String[] deltaSize,  boolean isInit, long time) throws ModelException{
		...
		try{ resourceTotalValue = Integer.parseInt(resourceTotal);
		}catch(NumberFormatException e){
		...
		if(! this.getEntityTyps().exist(defaultEntityType)){
			throw new ModelException("Model.createResource  ResourceId: "+id+"  DefaultEntityType does not exist: "+defaultEntityType);
		}
		...
		if(name.length > 0)	resource.setName(name[0]);
	...
	public void setResource(String id, String[] provide, String[] takeProcess, 
		...
		if(provide.length > 0){
			...
			if(tmp.length != 4)
				throw new ModelException("Model.setResource - provide  Provide must have the form: entityId|priority|resourceAnz|sortorder. Provide: "+provide[0]);
			provideId = tmp[0];
			if(! this.getEntities().exist(provideId))
				throw new ModelException("Model.setResource - provide  EntityId does not exist. EntityId: "+provideId);
			try{ prio = Integer.parseInt(tmp[1]);
			}catch(NumberFormatException e){
				throw new ModelException("Model.setResource - provide  priority isn't a Integer  priority: "+tmp[1]);
			}
			try{ resourceAnzValue = Integer.parseInt(tmp[2]);
			}catch(NumberFormatException e){
				throw new ModelException("Model.setResource - provide  resourceAnz isn't a Integer  anzVisible: "+tmp[1]);
			}
		...
		if(takeProcess.length > 0){
			if(! this.getEntities().exist(takeProcess[0]))
				throw new ModelException("Model.setResource - takeProcess  EntityId does not exist. EntityId: "+takeProcess[0]);
		...
		if(takeBackProcess.length > 0){
			...
			if(tmp.length != 2)
				throw new ModelException("Model.setResource - takeBackProcess  TakeBackProcess must have the form: entityId|resourceAnz. TakeBackProcess: "+takeBackProcess[0]);
			if(! this.getEntities().exist(tmp[0]))
				throw new ModelException("Model.setResource - takeBackProcess  EntityId does not exist. EntityId: "+tmp[0]);
			try{ resourceAnzValue = Integer.parseInt(tmp[1]);
			}catch(NumberFormatException e){
				throw new ModelException("Model.setResource - takeBackProcess  resourceAnz isn't a Integer  anzVisible: "+tmp[1]);
			}
	...
	public void createStock(String id, String[] name, String capacity, String initialUnits,
		...
		try{ capacityValue = Long.parseLong(capacity);
		}catch(NumberFormatException e){
		...
		try{ initialValue = Long.parseLong(initialUnits);
		}catch(NumberFormatException e){
		...
		if(! this.getEntityTyps().exist(defaultEntityType)){
			throw new ModelException("Model.createStock  StockId: "+id+"  DefaultEntityType does not exist: "+defaultEntityType);
		}
		...
		if(name.length > 0)	stock.setName(name[0]);
	...
	public void setStock(String id, String[] retrieveBegin, String[] retrieveEnd, 
		...
		if(retrieveBegin.length > 0){
			...
			if(tmp.length != 4)
				throw new ModelException("Model.setStock - retrieveBegin  RetrieveBegin must have the form: entityId|priority|resourceAnz|sortorder. RetrieveBegin: "+retrieveBegin[0]);
			retrieveId = tmp[0];
			...
			try{ produkteAnz = Integer.parseInt(tmp[2]);
			}catch(NumberFormatException e){
				throw new ModelException("Model.setStock - retrieve produkteAnz isn't a Integer  produkteAnz: "+tmp[1]);
			}
		...
		if(retrieveEnd.length > 0){
			if(! this.getEntities().exist(retrieveEnd[0]))
				throw new ModelException("Model.setStore - retrieveEnd  EntityId does not exist. EntityId: "+retrieveEnd[0]);
		...
		if(storeBegin.length > 0){
			...
			if(tmp.length != 4)
				throw new ModelException("Model.setStock - storeBegin  StoreBegin must have the form: entityId|priority|resourceAnz|sortorder. StoreBegin: "+storeBegin[0]);
			storeId = tmp[0];
		...
		if(storeEnd.length > 0){
			if(! this.getEntities().exist(storeEnd[0]))
				throw new ModelException("Model.setStore - storeEnd  EntityId does not exist. EntityId: "+storeEnd[0]);
	...
	public void createBin(String id, String[] name, String initialUnits, 
		...
		if(! this.getEntityTyps().exist(defaultEntityType)){
			throw new ModelException("Model.createBin  BinId: "+id+"  DefaultEntityType does not exist: "+defaultEntityType);
		}
		...
		if(name.length > 0)	bin.setName(name[0]);
	...
	public void setBin(String id, String[] retrieveBegin, String[] retrieveEnd, 
		...
		if(retrieveBegin.length > 0){
			tmp = Parameter.split(retrieveBegin[0]);
			if(tmp.length != 4)
				throw new ModelException("Model.setBin - retrieveBegin  RetrieveBegin must have the form: entityId|priority|resourceAnz|sortorder. RetrieveBegin: "+retrieveBegin[0]);
			retrieveId = tmp[0];
			if(! this.getEntities().exist(retrieveId))
			...
			try{ produkteAnz = Long.parseLong(tmp[2]);
			}catch(NumberFormatException e){
		...
		if(retrieveEnd.length > 0){
			if(! this.getEntities().exist(retrieveEnd[0]))
				throw new ModelException("Model.setBin - retrieveEnd  EntityId does not exist. EntityId: "+retrieveEnd[0]);
		...
		if(store.length > 0){
			...
			if(tmp.length != 2)
				throw new ModelException("Model.setBin - store  Store must have the form: entityId|resourceAnz. Store: "+store[0]);
			storeId = tmp[0];
			if(! this.getEntities().exist(storeId))
				throw new ModelException("Model.setBin - store  EntityId does not exist. EntityId: "+storeId);
			try{ produkteAnz = Integer.parseInt(tmp[1]);
			}catch(NumberFormatException e){
	...
	public void createWaitQueue(String id, String[] name,  
		...
		if(! this.getEntityTyps().exist(defaultEntityType)){
			throw new ModelException("Model.createWaitQueue  WaitQueueId: "+id+"  DefaultEntityType does not exist: "+defaultEntityType);
		}
		...
		try{
			...
			throw new ModelException("Model.createWaitQueue  WaitQueueId: "+id+"  parse Error deltaSize: "+deltaSize);
		}

		WaitingQueue waitingQueue = new WaitingQueue(this, id);
		if(name.length > 0)	waitingQueue.setName(name[0]);
	...
	public void setWaitQueue(String id, String[] insertMaster, String[] insertSlave, 
			String[] cooperationBegin, String[] cooperationEnd, boolean isInit, long time) throws ModelException{
		...
		if(insertMaster.length > 0){
			...
			if(tmp.length != 3)
				throw new ModelException("Model.setWaitQueue - insertMaster must have the form: masterId|priority|listRule. insertMaster: "+insertMaster[0]);
			masterId = tmp[0];
			try{ prio = Integer.parseInt(tmp[1]);
			}catch(NumberFormatException e){
				throw new ModelException("Model.setWaitQueue - insertMaster  priority is no Integer. Priority: "+tmp[1]);
			}
			listRule  = tmp[2];
			if(! this.getEntities().exist(masterId))
				throw new ModelException("Model.setWaitQueue - insertMaster  EntityId does not exist. EntityId: "+insertMaster[0]);
		...
		if(insertSlave.length > 0){
			...
			if(tmp.length != 3)
				throw new ModelException("Model.setWaitQueue - insertSlave must have the form: slaveId|listRule. insertSlave: "+insertSlave[0]);
			slaveId = tmp[0];
			try{ prio = Integer.parseInt(tmp[1]);
			}catch(NumberFormatException e){
				throw new ModelException("Model.setWaitQueue - insertSlave  priority is no Integer. Priority: "+tmp[1]);
			}
			listRule  = tmp[2];
			if(! this.getEntities().exist(slaveId))
				throw new ModelException("Model.setWaitQueue - insertSlave  EntityId does not exist. EntityId: "+insertSlave[0]);
		...
		if(cooperationBegin.length > 0){
			...
			if(tmp.length != 2)
				throw new ModelException("Model.setWaitQueue - cooperationBegin must have the form: masterId|slaveId. cooperationBegin: "+cooperationBegin[0]);
			masterId = tmp[0];
			slaveId  = tmp[1];
			if(! this.getEntities().exist(masterId))
				throw new ModelException("Model.setWaitQueue - cooperationBegin  MasterId does not exist. MasterId: "+masterId);
			if(! this.getEntities().exist(slaveId))
				throw new ModelException("Model.setWaitQueue - cooperationBegin  SlaveId does not exist. SlaveId: "+masterId);
		...
		if(cooperationEnd.length > 0){
			...
			if(tmp.length != 2)
				throw new ModelException("Model.setWaitQueue - cooperationEnd must have the form: masterId|slaveId. CooperationEnd: "+cooperationBegin[0]);
			masterId = tmp[0];
			slaveId  = tmp[1];
			if(! this.getEntities().exist(masterId))
				throw new ModelException("Model.setWaitQueue - cooperationEnd  MasterId does not exist. MasterId: "+masterId);
			if(! this.getEntities().exist(slaveId))
				throw new ModelException("Model.setWaitQueue - cooperationEnd  SlaveId does not exist. SlaveId: "+masterId);
	...
	public void createStatistic(String id, String[] name, String typeData, 
			...
			String[] isIntValue, String[] deltaSize, boolean isInit, long time) throws ModelException{
		...
		try{
			...
			throw new ModelException("Model.createStatistic StatisticId: "+id+" TypeData is no Integer. TypeData: "+typeData);
		}
		
		try{
			typeIndexValue = Integer.parseInt(typeIndex);
		}catch(NumberFormatException e){
			throw new ModelException("Model.createStatistic StatisticId: "+id+" TypeIndex is no Integer. TypeData: "+typeIndex);
		}
		...
		try{
			...
			throw new ModelException("Model.createStatistic  StatisticId: "+id+"  parse Error timeBounds: "+timeBounds);
		}
		...
		try{
			...
			throw new ModelException("Model.createStatistic  StatisticId: "+id+"  parse Error valueBounds: "+valueBounds);
		}

		try{
			histogramCellsValue = Integer.parseInt(histogramCells);
		}catch(NumberFormatException e){
			throw new ModelException("Model.createStatistic StatisticId: "+id+" HistogramCells is no Integer. HistogramCells: "+histogramCells);
		}
		...
		try{
			...
			throw new ModelException("Model.createStatistic StatisticId: "+id+" TypeAnimation is no Integer. TypeAnimation: "+typeAnimation);
		}
		...
		try{
			...
			throw new ModelException("Model.createStatistic  StatisticId: "+id+"  parse Error deltaSize: "+deltaSize);
		}
		...
		if(name.length > 0) statistic.setName(name[0]);
	 ...
	 * Long.MIN_VALUE means, end of simulation is given by
	 * stop role and not by time.
	 * @return
	 */
