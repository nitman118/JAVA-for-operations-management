...
<Proc extends EntityBasicAnimation> extends ModelComponent{
	 ...
	 * Each Entry contains some animated process entities.
	 * For each entry, the no of used resources are shown.
	...
	public ProcessStationResAbstrAnimation(ModelAnimation owner, String name, 
			String resName, int resTotal, Position pos, FormExt form, boolean showInAnimation){
		this(owner, name, null, resName, resTotal, pos, form, (ListInterface)null, showInAnimation);
	}
	...
	public ProcessStationResAbstrAnimation(ModelAnimation owner, String name, Comment comment,
			String resName, int resTotal, Position pos, FormExt form, boolean showInAnimation){
		this(owner, name, comment, resName, resTotal, pos, form, (ListInterface)null, showInAnimation);
	}
	...
	public boolean insert(java.util.List<Proc> procEntities, int needAnzRes){
		...
		for(int i=0; i<procEntities.size(); i++) entityIds[i] = procEntities.get(i).getName();
		if(this.showInAnimation){
			try {
