...
public class EntityGrafic extends JComponent implements Grafic, MouseListener{
	...
	private desmoj.extensions.visualization2d.engine.model.Entity 		entity;
	private desmoj.extensions.visualization2d.engine.model.EntityType	entityType;
	private Image 				image;
	private String 				code;
	private EntityPosition		positionIntern, positionExtern;
	 ...
	 * @return 	smallest Rectangle, when an static Entity exist.
	 * 			null otherwise 
	 */
	...
	public void transform(){
		if(this.positionExtern != null){
			...
			if(p == null){
				...
				EntityPosition p1 = new EntityPosition(p.x, p.y, positionExtern.getAngle(), positionExtern.getDirection());
	...
	public void setCode(){
		if(this.entity.getName() != null) 	this.code = this.entity.getName();
		else 								this.code = "id: "+this.entity.getId();
		this.setPreferredSize();
	}
	
	public void setLocation(EntityPosition position){
		this.positionIntern		= position;
		this.setLocation((int)Math.round(this.positionIntern.getX()-this.getWidth()/2), (int)Math.round(this.positionIntern.getY()-this.getHeight()/2));
