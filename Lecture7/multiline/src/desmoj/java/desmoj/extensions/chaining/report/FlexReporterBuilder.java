 ...
 * {@link FlexReporter}s which are special Reporters whose content 
...
public class FlexReporterBuilder {
	...
	public final class FlexReporter extends Reporter {
		 ...
		 * 
		 * @param groupHeading
		 *            The heading for the group this reporter contributes to
		 * @param groupId
		 *            The id of the group this reporter contributes to
		 * @param rows
	 ...
	 * For example by drawing it bold.
	 * 
	 * 
	 */
	...
	/**
	 * Represents a row that will be drawn in the report. A row is made up of
	 * individual cells
	 * 
	 */
	public class Row {
		 ...
		 * 
		 * @param closeCallback
		 *            A callback object that is notified when a row is closed
		 */
		public Row(RowCloseCallback closeCallback) {
			this.closeCallback = closeCallback;
			cells = new LinkedList<FlexReporterBuilder.Cell>();
		}
	...
	interface RowCloseCallback {
		abstract FlexReporterBuilder onRowClosed();
	}
