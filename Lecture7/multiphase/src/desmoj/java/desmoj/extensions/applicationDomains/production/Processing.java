 ...
 * a <code>WorkStation</code> where a <code>Worker</code> process or a
 ...
 * being processed are determined by a <code>PartsList</code>. The parts (or
 * products) are represented by <code>SimProcess</code> es. The
 ...
 * with the <code>WorkStation</code> construct, where a master process (
 ...
 * taking place at the <code>WorkStation</code> is described in the method
 * <code>cooperation</code>, which should be overwritten by the user to model
 * the manufacturing process taking place at the <code>WorkStation</code>.
 * Until now this method only models the time it takes to process the parts in
 * some way. Note: when using the <code>WorkStation</code> construct the
 ...
 * not been activated during the processing process already).
 * 
...
public class Processing extends ProcessCoop {
	 ...
	 * 
	 * @param owner
	 *            desmoj.Model : The model this Processing is associated to.
	 * @param name
	...
	/**
	 * This method describes the Processing process carried out by the master
	 * process (some kind of <code>Worker</code> or
	 * <code>MachineProcess</code>). In this simple case only the time it
	 * takes to process the parts is taken into consideration. If the user
	 * building the model has to implement a more complex behavior, he has to
	 * overwrite this method in a subclass. The time it takes to process the
	 * parts is obtained from the method <code>getProcessingTimeSample()</code>.
	 * 
	 * @param master
	 *            SimProcess : The master process which really carries out the
	 *            cooperation (and processes the slave parts). Should be a
	 *            subclass of <code>Worker</code> or
	 *            <code>MachineProcess</code>.
	 * @param slaves
	 *            SimProcess[] : The slave processes (parts) which are lead
	 *            through the process cooperation by the master.
	 */
	protected void cooperation(SimProcess master, SimProcess[] slaves) {
		/*
		...
		if (!((master instanceof desmoj.extensions.applicationDomains.production.Worker) || (master instanceof desmoj.extensions.applicationDomains.production.MachineProcess))) {
			sendWarning(
	 ...
	 * process the parts with the worker or machine. The time is taken from the
	 * given random number stream processingTimeStream.
	 * 
	...
	/**
	 * Sets the processingTimeStream to a new <code>RealDist</code> random
	 * number stream.
	 * 
