 ...
 * a certain number of products from an <code>Entrepot</code> to use them up.
 ...
 * method <code>lifeCycle()</code> in a subclass. The CustomerProcess can be
 * viewed as the border of the simulated system, where the products disappear.
 * It is intended to be used in conjunction with the <code>DemandProcess</code>.
 * CustomerProcess has two main parameters to be specified (see the
 * Constructor): The <code>Entrepot</code> where the products are to be
 * fetched from and the number of demanded products.
 * 
 * @see DemandProcess
 * 
...
public class CustomerProcess extends SimProcess {
	...
	/**
	 * Constructor for a CustomerProcess. The CustomerProcess is fetching a
	 * given number of products from the given <code>Entrepot</code>.
	 * 
	 * @param owner
	 *            desmoj.Model : The model this CustomerProcess is associated
	 *            to.
	 * @param name
	 *            java.lang.String : The name of this CustomerProcess.
	 * @param supplier
	 ...
	 * given <code>Entrepot</code>. As there are not enough products
	...
	public void lifeCycle() {
		...
		for (int i = 0; i < fetchedProducts.length; i++) {
			if (fetchedProducts[i] instanceof desmoj.core.simulator.ComplexSimProcess) {
