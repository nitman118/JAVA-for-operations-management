...
public class HistogramPlotter extends GraphicalObserver implements
	 ...
	 * @param context
	 *            graphical observer context the plotter is displayed in
	 * @param data
	 *            HistogramData consisting of double values to be displayed in
	 *            the histogram
	 * @param xUnit
	 ...
	 *            height of window
	 * @param x
	 *            horizontal window location
	 * @param y
	 *            vertical window location
	 */
	public HistogramPlotter(String title, GraphicalObserverContext context,
			Histogram data, String xUnit, int winWidth, int winHeight,
			int x, int y) {
	 ...
	 * @see desmoj.extensions.experimentation.ui.GraphicalObserver#getGUI()
	 */
	 ...
	 * Current HistogramData is displayed in chart until restarting the simulation.
	 * 
	 * @param e
	 *            a SimRunEvent
	...
	/**
	 * Calculates the upper border which should be displayed in the chart,
	 * and communicates all borders to the chart.
	 */
	private void calculateBars() {
		hasToDrawChart = true;
		myGUI.setMax_y((double)data.getObservationsInCell(data.getMostFrequentedCell())*100/data.getObservations());
		myGUI.setMax_x(data.getUpperLimit());
		myGUI.setMin_x(data.getLowerLimit(1));
		myGUI.redrawChart();
	}
