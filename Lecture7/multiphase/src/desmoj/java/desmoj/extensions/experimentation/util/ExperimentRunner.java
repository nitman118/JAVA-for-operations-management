...
/**
 * A thread a DesmoJ-Experiment can run in. The experiment runner can notifiy
 * registered ExperimentListeners when the assigned experiment is (re)started,
 * stopped or temporarily paused and registered SimClockListeners when the
...
public class ExperimentRunner implements Observer, Runnable, Parameterizable {
	...
	private static class Lock {
		...
		/**
		 * Tries to occupy the semaphore. If the semaphore is not available the
		 * calling thread has to wait in a lock.
		 */
	 ...
	 * settings
	 * 
	 * @param expSettingsMap
	 *            map of experiment settings
	 * @param modelParamMap
	...
	protected Experiment createExperiment() {
		...
		e = new Experiment(name, outputPath, 
		        AccessUtil.getTimeUnitValue(EXP_EPSILON, expSettings),
		        AccessUtil.getTimeUnitValue(EXP_REF_UNIT, expSettings), 
		        null,
		        reportOutputType,
...
//				"1.1.1970 00:00:00"));
	 ...
	 * the given experiment runner in an array. Returns an array of size 0 by
	 * default.
	 */
