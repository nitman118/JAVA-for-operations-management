 ...
 * legend items (if needed) and a special Chart object. All functionality of the Chart 
 * object which is needed by the user, is encapsulated in this class, so the user doesn`t 
 * need to have direct access to the Chart object, he doesn`t need to care about
 * the Chart object at all.
 * 
 * This class additionally includes a method saveAs(filename) for saving the chart to 
 * an image file.
 * 
 * If you are a user of DESMO-J you don`t need to create a object of this kind:
 * Create a HistogramPlotter or a TimeSeriesPlotter instead!
 * 
 * This class is needed by the graphical observers HistogramPlotter and TimeSeriesPlotter.
 * If you want to extend DESMO-J you can create new plotter classes inherited from
...
public class ChartPanel extends JPanel  implements ActionListener {
	...
	/**
	 * The chart class which is inherited from canvas. All chart functionality is implemented
	 * there.
	 */
	private Chart chart;
	 ...
	 * Create a HistogramPlotter or a TimeSeriesPlotter instead!
	 * 
	 * If you want to extend Desmo-J you can create new plotter classses inherited from
	 * GraphicalObserver which contains this object as a private field "myGUI"
	 * and a method drawChart(Graphics) for implementing the drawing of the data.
	 * 
	 ...
	 * If saving fails, the exception will be catched, but not handled.
	 * @param title
	 */
	...
	/**
	 * Returns the colors used to draw charts. You can draw more than one diagram in one
	 * chart. If you do so, you have to use colors in the same order, as the legend showcases them.
	 * This static method returns different colors, starting with 0=RED and ending with 
	 * 9=LIGHTGREY. All values higher or smaller than this will return BLACK.
	 * This is not implemented as static final array, because of the possibility to draw
	 * more charts then colors exists (they will be black).
