...
/**
 * A message which a process can send to a {@link MessageCrossbar} or a
 * {@link MessageChannel} to activate processes waiting on the them. The
 * CrossbarMessage contains the {@link TimeInstant} on which it was sent and its
 * sender (the process that send the message to the crossbar).<br />
 * <br />
 * Subclasses of the CrossbarMessage can be used to transport information
...
public class CrossbarMessage extends Message implements Cloneable {
	 ...
	 * receiver processes. It can either be distributed by reference, meaning
	 * each receiver gets the exact same message object, or as a copy so that
	 * each receiver only gets a (shallow) copy of the original message object.
	 * 
	 * 
	 * @author Malte Unkrig
	 * 
	...
	/**
	 * The sender process of the CrossbarMessage. The sender is automatically
	 * set on send time by the message crossbar.
	 */
	private SimProcess sender;

	/**
	 * The mode defining how the message will be distributed to receiver
	 * processes.
	 */
	private final DistributionMode distributionMode;

	/**
	 * Constructs a CrossbarMessage with the given model and description. This
	 * message will be distributed to receivers by reference. To change that
	 * specify a {@link DistributionMode} using the constructor
	 * {@link CrossbarMessage#CrossbarMessage(Model, String, DistributionMode)}
	 * 
	 * 
	 * @param origin
	 *            The model the CrossbarMessage belongs to.
	 * @param description
	 *            The description for the message.
	 */
	 ...
	 *            The {@link DistributionMode} for this message defining whether
	 *            it will be distributed to receivers by reference or as a copy.
	 */
	...
	/**
	 * 
	 * @return The DistributionMode of this message defining whether it will be
	 *         passed to receiver processes by reference or as a copy.
	 */
