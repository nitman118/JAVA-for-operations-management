...
public class ProcessNewGrafic extends JPanel implements Grafic{
	...
	private Point			pointIntern, pointExtern;
	private boolean			printable;
	private JScrollPane 	p;
	private JPanel 			a, b;
	private JLabel 			t, usedRes;
	private boolean 		horizontal;
	private Dimension 		entityDimension;
	private int 			anzVisible;
	...
	private boolean			showResources; // Ressourcen sollen animiert werden
	...
	public static String[] getViews(Model model){
		...
		for(int i=0; i<ids.length; i++){
			...
			if(processGrafic != null){
				...
				if(! views.contains(viewId)) views.add(viewId);
			}
		}
	 ...
	 * @return 	smallest rectangle when an ProcessNewGrafic exist,
	 * 			null otherwise.
	 */
	public static Rectangle getBoundsExternGlobal(Model model, String viewId){
		...
		Rectangle r = null;
		if(found) r = new Rectangle(minX, minY, maxX-minX, maxY-minY);
	...
	public static void updateInit(Model model, String viewId, JComponent processPanel, JComponent linePanel){
		...
		for(int i=0; i<id.length; i++){
			...
			if(processGrafic != null && 
				...
				if(processLineList != null) linePanel.add(processLineList);
	 ...
	 * @param horizontal			true, when entities are horizontal arranged.
	 * 								false. when vertical
	...
	public ProcessNewGrafic(ProcessNew process, String viewId, Point pointExtern, 
		...
		if(process.getName() != null) 	this.code = this.process.getName();
		else 						this.code = "id: "+this.process.getId();
		...
		if(deltaSize != null)	this.deltaSize = deltaSize;
		this.showResources		= showResources;
		
		a = new JPanel(); a.setLayout(new FlowLayout(FlowLayout.LEFT,0,0));
		a.setBackground(ModelGrafic.COLOR_BACKGROUND);
		b = new JPanel(); a.add(b);
		b.setOpaque(false);
		...
		JPanel corner = new JPanel(); corner.setLayout(new GridLayout(1,1)); corner.setBackground(Grafic.COLOR_BACKGROUND);
		this.dh = 0;
		...
		if(this.horizontal){
			JLabel res = new JLabel("Res.", JLabel.RIGHT); res.setFont(Grafic.FONT_DEFAULT);
			...
			JLabel proc = new JLabel("Proc.", JLabel.RIGHT); proc.setFont(Grafic.FONT_DEFAULT);
			...
			headerH.add(res);
			if(!process.isAbstractResource())headerH.add(Box.createVerticalGlue());
			headerH.add(proc);
			...
			if(this.showResources) p.setRowHeaderView(headerH);
			...
			if(commentLabel != null && this.process.isCommentSizeExt()){
				int r = this.getInsets().left + this.getInsets().right + commentLabel.getInsets().left + commentLabel.getInsets().right;
				d1 = new Dimension(Math.max(d.width, commentLabel.getPreferredSize().width)+r, d.height + commentLabel.getPreferredSize().height);
			}
			this.setBounds(this.pointIntern.x-d1.width/2, this.pointIntern.y-d1.height/2, d1.width, d1.height);

		}else{
			JLabel res = new JLabel("Res.", JLabel.LEFT); res.setFont(Grafic.FONT_DEFAULT);
			Dimension resD = res.getPreferredSize(); 
			res.setPreferredSize(new Dimension(this.entityDimension.width, resD.height));
			JLabel proc = new JLabel("Proc.", JLabel.LEFT); proc.setFont(Grafic.FONT_DEFAULT);
			Dimension procD = proc.getPreferredSize(); 
			proc.setPreferredSize(new Dimension(this.entityDimension.width, procD.height));
			JPanel headerV = new JPanel(); headerV.setLayout(new GridLayout(1,1));
			headerV.setBackground(Grafic.COLOR_BACKGROUND);
			headerV.setLayout(new FlowLayout(FlowLayout.CENTER));
			headerV.add(res); headerV.add(proc);
			...
			if(this.showResources) p.setColumnHeaderView(headerV);
			p.setCorner(JScrollPane.UPPER_RIGHT_CORNER, corner);
			javax.swing.JScrollBar sb = p.getVerticalScrollBar();
			sb.setPreferredSize(new java.awt.Dimension(5, sb.getPreferredSize().height));
			...
			Dimension d1 = d;
			if(commentLabel != null && this.process.isCommentSizeExt()){
				int r = this.getInsets().top + this.getInsets().bottom + commentLabel.getInsets().top + commentLabel.getInsets().bottom;
				d1 = new Dimension(d.width + commentLabel.getPreferredSize().width, Math.max(d.height, commentLabel.getPreferredSize().height +r));
			}
			this.setBounds(this.pointIntern.x-d1.width/2, this.pointIntern.y-d1.height/2, d1.width, d1.height);
		}
	 ...
	 * Will be called from ProcessNew.addEntry and ProcessNew.removeEntry.
	 */
	public void update(){
		...
		if(this.showResources){
			...
			for(int i=0; i<anzEntry; i++){
				if(this.process.isAbstractResource()){
					...
					JLabel l = new JLabel(Integer.toString(r)+" "+this.process.getResourceType());
	...
	private Dimension maxEntityDimension(){
		...
		for(int i=0; i<this.process.getEntriesAnz(); i++){
			String[] procs = this.process.getProcessEntries(i);
			for(int j=0; j<procs.length; j++){
				desmoj.extensions.visualization2d.engine.model.Entity en = process.getModel().getEntities().get(procs[j]);
			...
			if(!this.process.isAbstractResource()){
				String[] res = this.process.getResourceEntries(i);
				for(int j=0; j<res.length; j++){
					desmoj.extensions.visualization2d.engine.model.Entity en = process.getModel().getEntities().get(res[j]);
					width 	= Math.max(width,  ((EntityGrafic)en.getGrafic()).getWidth());
					height 	= Math.max(height, ((EntityGrafic)en.getGrafic()).getHeight());
				}
			}
	...
	public ProcessLineList getProcessLineList(){
		...
		if(li != null){
			ListGrafic listGrafic = (ListGrafic)process.getModel().getLists().get(this.process.getListId()).getGrafic();
			if(listGrafic != null){
				Point list		= new Point(listGrafic.getX()+listGrafic.getWidth()/2, listGrafic.getY()+listGrafic.getHeight()/2);
	...
	/**
	 * Dispatches mouse event to EntityGrafic.
	 * Called from all mouse event handler.
	 * @param e
	 */
	private void mouseEventHandler(MouseEvent e){
		...
		for(int i=0; i<comps.length; i++){
			if(comps[i] instanceof desmoj.extensions.visualization2d.engine.modelGrafic.EntityGrafic) {
				desmoj.extensions.visualization2d.engine.modelGrafic.EntityGrafic en = (desmoj.extensions.visualization2d.engine.modelGrafic.EntityGrafic)comps[i];
				if(en.getBounds().contains(e.getPoint())){
					en.dispatchEvent(e); break;
				}
			}
		}
	}
	public void mousePressed(MouseEvent e)	{mouseEventHandler(e);}
	public void mouseReleased(MouseEvent e)	{mouseEventHandler(e);}
	public void mouseClicked(MouseEvent e)	{mouseEventHandler(e);}
	public void mouseEntered(MouseEvent e)	{mouseEventHandler(e);}
	public void mouseExited(MouseEvent e)	{mouseEventHandler(e);}
	...
	class ProcessLineList extends JComponent{
		...
		public void paintComponent(Graphics g){
			...
			g2.drawLine(this.process.x-this.min.x+2, this.process.y-this.min.y+2, this.list.x-this.min.x+2, this.list.y-this.min.y+2);
		}
	}
