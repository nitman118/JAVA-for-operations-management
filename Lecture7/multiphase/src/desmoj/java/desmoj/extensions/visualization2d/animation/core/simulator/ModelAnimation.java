...
/**
 * Animated version of model. ModelAnimation extends Model about
 * a CmdGeneration instance and a showInAnimation switch.
...
public abstract class ModelAnimation extends Model {
	...
	private Vector<EntityTypeAnimation>	entityTypes;
	
	
	/**
	 * ModelAnimation extends Model about a CmdGeneration
	 * instance and a showInAnimation switch.
	 * In constructor you must define: setModelAuthor(..), 
	 * setModelDate(), addRemark(), addIcon() and setGeneratedBy()
	 ...
	 * Originally, before experiment start presentTime is 0. 
	 * Now, in initPhase presentTime is initTime (from cmdGeneration object) 
	 */
	public TimeInstant presentTime(){
		TimeInstant out = super.presentTime();
		if(this.cmdGen.isInitPhase()) out = this.cmdGen.getInitTime();
		return out;
	}
	...
	protected String getRemark(){
		String out = "";
		for(int i=0; i<this.remarks.size(); i++) out += this.remarks.get(i)+" ";
		return out;
	}
	...
	/**
	 * generate "createModelBasisData" Command
	 * and EntityTypeAnimation Commands
	 * used by init()
	 */
	...
	/**
	 * check setting of modelAuthor, modelDate and remarks
	 * used by initCmds()
	 */
