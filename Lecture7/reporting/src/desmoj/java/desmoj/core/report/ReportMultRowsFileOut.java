...
public class ReportMultRowsFileOut extends ReportFileOut {
	...
	public void receive(desmoj.core.report.Reporter r) {
		...
		if (Reporter.isOtherGroup(r, lastReporter)
			...
			//---added by Johanna Djimandjaja
			if (formatter instanceof HTMLTableChartFormatter && r.makeAdditionalColorEntryIfHTMLColorChartIsGenerated()) {
		...
		// normal handling of all other Reporters
		{
			...
			if (r instanceof TimeSeriesReporter &&
				...
				if (_timeSeriesCanvas.get(data.getGroup()) == null) {
				    ...
				    CanvasTimeSeries c = new CanvasTimeSeries("timeSeriesCanvas" + chartFormatter.getFreeCanvasIDNum(), 350, 500, tsReporter.getChartData(), "time", yAxis);
	...
	private void innerReport(HistogramReporter hr){
		...
		{	
			...
			if (formatter instanceof HTMLTableChartFormatter) {
				...
				histCanvas = new CanvasHistogramLong("histogramCanvas" + chartFormatter.getFreeCanvasIDNum(), 350, 500, histData, hr.source.getName());
			}
			...
			if (chartFormatter != null) formatter.writeHeadingCell("Color");
			else formatter.writeHeadingCell(histTitleBuf[hr.getHistNumColumns()-1]);
			...
			for (int j = 0; j < numOfRows; j++) {
				...
				if (chartFormatter != null) chartFormatter.writeColoredCell(histCanvas.getDataColor(j));
				else formatter.writeCell(histEntryBuf[j][hr.getHistNumColumns()-1], 1);
	...
	private void innerReport(HistogramAccumulateReporter hr){
		...
		{	
			...
			if (formatter instanceof HTMLTableChartFormatter) {
				...
				histCanvas = new CanvasHistogramDouble("histogramAccumulateCanvas" + chartFormatter.getFreeCanvasIDNum(), 350, 500, histData, hr.source.getName());
			}
	...
	private void innerReport(TextHistogramReporter thr){
		...
		{		
			...
			if (formatter instanceof HTMLTableChartFormatter) {
				...
				textHistCanvas = new CanvasHistogramLong("textHistogramCanvas" + chartFormatter.getFreeCanvasIDNum(), 350, 500, textHistData, thr.source.getName());
			}
			...
			//if (formatter instanceof HTMLTableChartFormatter) change the last heading to "Color"
			if (chartFormatter != null) formatter.writeHeadingCell("Color");
			else formatter.writeHeadingCell(textHistTitleBuf[thr.getTextHistNumColumns()-1]);
			...
			for (int j = 0; j < numOfRows; j++) {
				...
				//if (formatter instanceof HTMLTableChartFormatter) change the last content to a colored cell
				if (chartFormatter != null) chartFormatter.writeColoredCell(textHistCanvas.getDataColor(j));
				else formatter.writeCell(histEntryBuf[j][thr.getTextHistNumColumns()-1], 1);
