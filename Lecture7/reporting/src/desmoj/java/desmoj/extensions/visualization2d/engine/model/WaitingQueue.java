...
public class WaitingQueue implements Basic {
	...
	private	List						slaves;
	private java.util.List<Cooperation> cooperations;
	...
	public WaitingQueue(Model model, String id){
		...
		this.cooperations			= new java.util.LinkedList<Cooperation>();
		if(this.id != null) model.getWaitingQueues().add(this);
	}
	...
	public void cooperationBegin(String masterId, String slaveId, long time){
		...
		if(this.masters.containsInContainer(masterId) ){
			...
			throw new ModelException("WaitingQueue.cooperationBegin: Master isn't in masters waiting queue. MasterId: "+masterId);
		}
		if(this.slaves.containsInContainer(slaveId)){
			this.remove(slaveId, false, time);
		}else{
			throw new ModelException("WaitingQueue.cooperationBegin: Slave isn't in slaves waiting queue. SlaveId: "+slaveId);
		}
	...
	public void cooperationEnd(String masterId, String slaveId, long time){
		...
		if(! found)
			throw new ModelException("WaitingQueue.cooperationEnd: The master-slave couple isn't in a actual cooperation. MasterId: "+masterId+"  SlaveId: "+slaveId);
		((WaitingQueueGrafic)this.getGrafic()).update();
	}
	...
	public Grafic createGrafic(String viewId, int x, int y, String defaultEntityTypeId, 
		...
		this.grafic =  new WaitingQueueGrafic(this, viewId, new Point(x, y), defaultEntityTypeId, anzVisible, horizontal, deltaSize);
		return this.grafic;
	}
	...
	class Cooperation{
		...
		public Cooperation(String masterId, String slaveId){
			...
			if(! model.getEntities().exist(masterId))
				throw new ModelException("animation.model.WaitingQueue.Cooperation: MasterId don't exist! (master, slave): ("+masterId+","+slaveId+")");
			if(! model.getEntities().exist(slaveId))
				throw new ModelException("animation.model.WaitingQueue.Cooperation: SlaveId don't exist! (master, slave): ("+masterId+","+slaveId+")");
		}
