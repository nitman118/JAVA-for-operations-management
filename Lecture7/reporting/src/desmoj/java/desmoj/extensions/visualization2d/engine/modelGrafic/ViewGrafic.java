...
/**
 * The Viewer contains TabedPane with different views. Each view
 * needs a viewGrafic.
 * 
...
public class ViewGrafic extends JLayeredPane implements Grafic {
	...
	public ViewGrafic(Model model, String viewId) throws ModelGraficException{
		...
		if(this.rectangleExtern == null) this.rectangleExtern = new Rectangle(0,0,100, 50);
		...
		this.max				= new Point(this.rectangleExtern.x + this.rectangleExtern.width, this.rectangleExtern.y + this.rectangleExtern.height);
		...
		if((this.min.x > this.max.x) || (this.min.y > this.max.y))
			throw new ModelGraficException("ModelGrafic Min-point is not componetwise less max-point. min: ("+this.min.x+", "+this.min.y+")  max: ("+this.max.x+", "+this.max.y+") ");
		
		// interne Sicht, verschiebe Ecke links oben nach (0,0)
		this.rectangleIntern	= new Rectangle(0,0, (int)(this.zoomFactor* this.rectangleExtern.width)+2*ModelGrafic.BOUNDARY_WIDTH, 
	 ...
	 * Title is printed in top left corner, when title != null
	 * @param title
