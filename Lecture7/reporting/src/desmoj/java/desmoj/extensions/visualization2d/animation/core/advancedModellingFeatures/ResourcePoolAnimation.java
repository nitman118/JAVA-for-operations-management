...
public class ResourcePoolAnimation extends Res{
	 ...
	 * @param sortOrder			of inout queue possible values:
	...
	public void createAnimation(Position pos, FormExt form, String resourceType, 
		...
		if(this.showInAnimation){
			try {
				if(init)	c = Command.getCommandInit("createResource", this.cmdGen.getAnimationTime(simTime));
				else 		c = Command.getCommandTime("createResource", this.cmdGen.getAnimationTime(simTime));
	 ...
	 * It want to move into input queue and wait to processing.
	 * @param proc
	 * @param needRes
	 * @return			true:  its moved in input queue
	 * 					false: its rejected
	 */
	 public boolean provide(EntityBasicAnimation proc, int needRes){
		boolean out;
		TimeInstant simTime = this.getModel().presentTime();
		boolean	init	= this.cmdGen.isInitPhase();
		Command c;
		String[] provide = {proc.getName(), Integer.toString(proc.getPriority()), Integer.toString(needRes), this.priorityAttribute};
		if(this.showInAnimation){
			try {
				if(init)	c = Command.getCommandInit("setResource", this.cmdGen.getAnimationTime(simTime));
				else 		c = Command.getCommandTime("setResource", this.cmdGen.getAnimationTime(simTime));
				c.addParameter("ResourceId", this.id);
	 ...
	 * when all resources are given back, it leaves the resource pool.
	 * @param proc
	 * @param takeBackRes
	 */
	 ...
	 * 					false: its rejected
	 */
	public boolean provide(int needRes){
