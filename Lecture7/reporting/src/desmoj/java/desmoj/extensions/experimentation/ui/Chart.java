 ...
 * There a methods (drawLine, drawRect, fillRect) to paint on the chart
 * without taking care about transforming or scaling the dimensions.
 * You have to add this class to a panel or a frame.
 * You also have to implement the method which should draw the data (e.g. a
...
public class Chart extends JComponent implements ActionListener {
	...
	/**
	 * The object who implements the method for drawing the data.
	 * In Desmo-J this is the HistogramPlotter respectively the 
	 * TimeSeriesPlotter.
	 */
	...
	/** The left border of the white space for drawing the chart.
	 * This is not 0, because in the left of the chart there are
	 *  for example the labels of the y axis. */
	int borderLeft;
	
	/** The right border of the white space for drawing the chart.
	 * This is not equal to the width of the canvas, because in the right of the chart
	 * there is a small spacing. */
	int borderRight;
	
	/** The upper border of the white space for drawing the chart.
	 * This is not 0, because above the chart there is a small spacing. */
	int borderTop;
	
	/** The right border of the white space for drawing the chart.
	 * This is not equal to the width of the canvas, because under the chart
	 * there are for example the labels of the x axis. */
	int borderBottom;
	...
	/** 
	 * Constructor of the chart. Saves the given labels for 
	 * x- and y-axis and calls the constructor of java.awt.Canvas
	 ...
	 * You don`t have to do this! It overrides the paint(Graphics) method 
	 * from java.awt.Canvas. If size of the chart is big enough, 
	 * it calls paintBackground() (for painting the chart background).
	...
	/**
	 * Draws the chart basics. Calculates the best fitting intervalls
	 * for the axes. Paints a white backgorund, the axes, the labels
	 * and a grey grid for easier metering.
	 * 
	 * At last it tansforms (not scales!) the coordinate system. Thus the methods drawLine, drawRect 
	 * and fillRect must not take care about transforming: The root of the coordinate system 
	 * will be (0,0), and the value y will be autoscaled to -y, so a higher data value will 
	 * be painted above a lower y value, unlike the coordinate system of a canvas.
	 * ATTENTION: The value "1" still represents a pixel, because if we would scale "1" to
	 * interval_x (or rather interval_y) a new line would be very fat (that is not want
	 * we want). The "scaling" will be done by the methods calculateXPosition() and calculateYPosition()
	 * of this class, which will be automatically invoked if you use drawLine(), drawRect() or fillRect().
	 * @param g The specified Graphics context.
	 */
	public void paintBackground(Graphics g) {		
		...
		String[] label;	       // helping variable for drawing the labels (saves the string representation of every label)
		...
		int [] y = new int[3];
		x[0] = borderLeft;		y[0] = borderTop-arrowWidth;
		x[1] = borderLeft-arrowWidth;	y[1] = borderTop;
		x[2] = borderLeft+arrowWidth;	y[2] = borderTop;
		g.fillPolygon(x, y, 3);  // arrow on y-axis
		x[0] = borderRight+arrowWidth;		y[0] = borderBottom;
		x[1] = borderRight;	y[1] = borderBottom-arrowWidth;
		x[2] = borderRight;	y[2] = borderBottom+arrowWidth;
		g.fillPolygon(x, y, 3);  // arrow on x-axis
		
//		 paints the label (header) for the y axis
		g.setColor(Color.black);
		drawVerticalString((Graphics2D)g, fdr, y_label, (float)(fontHeight*1.25), (float)((borderBottom+font.getStringBounds(y_label, fdr).getWidth())/2));
		g.drawString(x_label, borderLeft+(int)((borderRight-borderLeft-font.getStringBounds(x_label, fdr).getWidth())/2), this.getHeight()-1 );
	 ...
	 * and calculateYPosition() so you don`t have to take care about any scaling or transforming.
	 * @param g The specified Graphics context.
	...
	/**
	 * Draws an unfilled rectangle into the chart. This method automatically calls the methods calculateXPosition()
	 * and calculateYPosition() so you don`t have to take care about any scaling or transforming.
	 * @param g The specified Graphics context.
	 * @param x1 The x value of the starting coordinates.
	 * @param y1 The y value of the starting coordinates.
	...
	/**
	 * Draws a filled rectangle into the chart. This method automatically calls the methods calculateXPosition()
	 * and calculateYPosition() so you don`t have to take care about any scaling or transforming.
	 * @param g The specified Graphics context.
	 * @param x1 The x value of the starting coordinates.
	 * @param y1 The y value of the starting coordinates.
	 ...
	 * and calculateYPosition() so you don`t have to take care about any scaling or transforming.
	 * 
	 * If you have a graphic context (e.g. method shall be invoked via a paint(Graphics g) method
	 * you should NOT use this method, but rather drawLine(Graphics, double, double, double, double).
	 ...
	 * The pair will NOT be stored, it only will be tested if the
	 * values are lower or higher than the existing minimums and
	 * maximums. This is needed for scaling the chart automatically.
