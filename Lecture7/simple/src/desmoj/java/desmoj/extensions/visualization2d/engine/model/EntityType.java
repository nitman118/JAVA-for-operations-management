...
public class EntityType implements ClassBasic {
	...
	private int 									height;
	private int										show;
	private TreeMap<String, String> 				possibleStates;
	private TreeSet<String> 						possibleAttributes;
	private Model 									model;
	 ...
	 */
	public EntityType(String id, desmoj.extensions.visualization2d.engine.model.Model model, int width, int height, 
		...
		if (id != null) model.getEntityTyps().add(this);
	}
	...
	public desmoj.extensions.visualization2d.engine.model.Model getModell(){
		return this.model;
	}
	...
	public String getImageId(String state){
		...
		if(this.possibleStates.containsKey(state)) out = this.possibleStates.get(state);
		return out;
	}
