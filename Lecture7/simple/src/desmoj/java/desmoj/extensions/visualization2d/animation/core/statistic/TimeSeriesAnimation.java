...
public class TimeSeriesAnimation extends TimeSeries{
	 ...
	 * @param end					end time
	 * @param showInTrace			as in TimeSeries
	 */
	public TimeSeriesAnimation(ModelAnimation owner, String name, 
			TimeInstant start, TimeInstant end,  boolean showInTrace) {
		
		super(owner, name, start, end, showInTrace);
	...
	public void createAnimation(Position pos, Form form, boolean showInAnimation) {
		...
		this.createAnimation(0.0, 1.0, 10, pos, form, showInAnimation);
	}
	 ...
	 * @param showInAnimation	switch animation on or off
	 */
	public void createAnimation(double valMin, double valMax, int histogramCells, 
			Position pos, Form form, boolean showInAnimation) {
		...
		timeMin = this.start.getTimeRounded(TimeUnit.MILLISECONDS);
		timeMax = this.end.getTimeRounded(TimeUnit.MILLISECONDS);
		...
		if(this.showInAnimation){
			try {
				...
				c.addParameter("TypeAnimation", StatisticGrafic.ANIMATION_TimeValueDiagram+"");
				//c.addParameter("IsIntValue", "");
				c.addParameter("DeltaSize", Parameter.cat(deltaSize1));
				c.setRemark(this.getGeneratedBy(TimeSeriesAnimation.class.getSimpleName()));
