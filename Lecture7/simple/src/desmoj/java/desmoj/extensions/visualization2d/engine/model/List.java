 ...
 * Grafical properties are stired in ListGrafic
 * 
...
public class List implements Basic{
	...
	private java.util.LinkedList<Entity>  	content;
	...
	public List(Model model, String praefix, String id){
		...
		this.id					= praefix+id;
		this.name				= null;
		if(this.id != null) model.getLists().add(this);
	...
	public Grafic createGrafic(String viewId, int x, int y, 
			...
			boolean horizontal){
		this.grafic = new desmoj.extensions.visualization2d.engine.modelGrafic.ListGrafic(this, viewId,
	 ...
	 * @throws ModelException
	 */
	public boolean addToContainerAfter(String entityId, int priority, String entityAfterId, long time)throws ModelException{
		boolean out = false;
		Entity entity = model.getEntities().get(entityId);
		// update priority attribute
		if(priority != entity.getPriority()){
	 ...
	 * @throws ModelException
	 */
	public boolean addToContainerBefore(String entityId, int priority, String entityBeforeId, long time)throws ModelException{
		boolean out = false;
		Entity entity = model.getEntities().get(entityId);
		// update priority attribute
		if(priority != entity.getPriority()){
