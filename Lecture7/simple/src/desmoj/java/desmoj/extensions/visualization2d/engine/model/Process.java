...
public class Process implements Basic {
	...
	private String 		id;
	private String 		name;
	private int 		anz_ResourceEntities;
	private int 		anz_ProcessEntities;
	private String[] 	resourceEntityIds;
	private String[] 	prozessEntityIds;
	...
	public Process(Model model, String id, int anz_ResourceEntities, int anz_ProcessEntities, String listId){
		...
		for(int i=0; i<anz_ResourceEntities; i++) this.resourceEntityIds[i] = " ";
		for(int i=0; i<anz_ProcessEntities; i++)  this.prozessEntityIds[i] = " ";
		this.listId					= listId;
		if(this.id != null) model.getProcesses().add(this);
	}
	...
	public void setProzessEntity(int index, String entityId, long time)throws ModelException{
		if((index >= 0) && (index < this.anz_ProcessEntities))
		{
			if(model.getEntities().get(entityId).isFree()){
				...
				throw new ModelException("model.Process.setProcessEntity Entity is not free: ProcessId:"+this.getId()+"  EntityId:"+entityId);
			}
		}else{
			throw new ModelException("model.Process.setProcessEntity Index is not valid: "+index);
		}
		if(this.grafic != null) ((ProcessGrafic) this.grafic).update();
		//this.printEntries();
	}
	...
	public void unsetProzessEntity(int index, String entityId, long time)throws ModelException{
		if((index >= 0) && (index < this.anz_ProcessEntities))
		{
			if(this.prozessEntityIds[index].equals(entityId)){
				...
				throw new ModelException("model.Process.unsetProcessEntity index and EntityId are not compatibel: ProcessId:"+this.getId()+"  EntityId:"+entityId+"  Index: "+index);
			}
		}else{
	...
	public void setResourceEntity(int index, String entityId, long time)throws ModelException{
		if((index >= 0) && (index < this.anz_ResourceEntities))
		{
			if(model.getEntities().get(entityId).isFree()){
				...
				throw new ModelException("model.Process.setResourceEntity Entity is not free: ProcessId:"+this.getId()+"  EntityId:"+entityId);
			}
		}else{
	...
	public void unsetResourceEntity(int index, String entityId, long time)throws ModelException{
		if((index >= 0) && (index < this.anz_ResourceEntities))
		{
			if(this.resourceEntityIds[index].equals(entityId)){
				...
				throw new ModelException("model.Process.unsetResourceEntity index and EntityId are not compatibel: ProcessId:"+this.getId()+"  EntityId:"+entityId+"  Index: "+index);
			}
		}else{
