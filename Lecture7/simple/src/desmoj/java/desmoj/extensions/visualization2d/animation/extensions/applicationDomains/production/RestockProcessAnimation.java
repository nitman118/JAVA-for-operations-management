...
public abstract class RestockProcessAnimation{
	...
	public void createAnimation(String iconId, boolean showInAnimation){
		...
		if(showInAnimation){
			try {
				if(init)	c = Command.getCommandInit("createEntity", cmdGen.getAnimationTime(simTime));
				else 		c = Command.getCommandTime("createEntity", cmdGen.getAnimationTime(simTime));
