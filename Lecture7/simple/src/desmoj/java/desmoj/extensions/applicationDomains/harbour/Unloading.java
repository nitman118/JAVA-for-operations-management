 ...
 * Unloading is used with the <code>WaitQueue</code> construct, where a
 ...
 * unload the truck/ an internal transporter.
 * 
...
public class Unloading extends ProcessCoop {
	 ...
	 * determined by the specified unloadTimeStream.
	 * 
	 * @param owner
	 *            desmoj.Model : The model this Unloading is associated to.
	 * @param name
	...
	/**
	 * This method describes the unloading process carried out by the master
	 * process (some kind of <code>InternalTransporter</code> or
	 * <code>Crane</code>). In this simple case only the time it takes to
	 * unload the containers (goods) is taken into consideration. If the user
	 ...
	 * 
	 * @param master
	 *            SimProcess : The master process which really unloads the
	 ...
	 * unload the containers (goods) with the transporter or crane. The time is
	 * taken from the given random number stream unloadTimeStream.
	 * 
	 * @return desmoj.SimTime : The time it takes to unload the containers
	 *         (goods) with the transporter/crane.
	 */
	...
	/**
	 * Sets the unloadTimeStream to a new <code>RealDist</code> random number
	 * stream.
	 * 
	 * @param unloadTimeStream
