 ...
 * (or the product process itself) using the <code>storeProduct()</code>
 * method. Products can be retrieved (removed) from the Entrepot using one of
 * the <code>removeProduct()</code> methods. If no or not enough products are
...
public class Entrepot extends QueueBased {
	...
	/**
	 * Weighted sum of available products in the Entrepot over the time (must be
	 ...
	 * and retrieved. The queueing discipline and the capacity of the underlying
	 * customer queue can be chosen.
	 * 
	 * @param owner
	 *            desmoj.Model : The model this Entrepot is associated to.
	 * @param name
	 *            java.lang.String : The name of this Entrepot.
	 * @param sortOrder
	...
	/**
	 * Constructs an empty Entrepot where products (SimProcesses) can be stored
	 * and retrieved. The underlying queue has a FIFO queueing discipline and
	 * unlimited capacity.
	 * 
	 * @param owner
	 *            desmoj.Model : The model this Entrepot is associated to.
	...
	/**
	 * Activates the first process waiting in the customers' queue. That is a
	 ...
	 * 
	 * @return double : The average number of products available in the Entrepot
	 ...
	 * 
	 * @return boolean :<code>true</code> if and only if the specified
	 ...
	 * which are in compliance with the given <code>Condition</code>. This
	 ...
	 * 
	 * @return int : The number of products available in the Entrepot at the
	 *         moment which are in compliance with the given
	 *         <code>Condition</code>.
	 * @param condition
	 *            desmoj.Condition : The <code>Condition</code> the products
	 ...
	 * Entrepot.
	 * 
	 * @return long : The number of customers having obtained products from the
	 *         Entrepot.
	 */
	 ...
	 * 
	 * @return long : The number of SimProcesses refused to be enqueued in the
	 *         queue.
	 */
	 ...
	 * 
	 * @return long : The number of products which already have been sold
	 *         (passed through this Entrepot).
	 */
	 ...
	 * 
	 * @return boolean :<code>true</code> if and only if this Entrepot has no
	 *         products stored (it is empty); <code>false</code> otherwise.
	 */
	 ...
	 * 
	 * @return java.util.Enumeration : An enumeration of the products in this
	 *         Entrepot.
	 */
	 ...
	 * in the queue. Note, that as long as the lifeCycles of the products
	 * (SimProcesses) leaving the Entrepot are not terminated they will
	 * automatically be activated after the current SimProcess! If no products
	 * are available at the moment <code>null</code> will be returned. The
	 * products will be retrieved in a kind of FIFO order, that means the
	 * product which arrived first in the Entrepot is the first to be removed.
	 * 
	 * @return desmoj.SimProcess[] : All the products contained in the Entrepot
	 *         at the moment will be removed. The lifeCycles of their
	 *         SimProcesses will be activated as long as they are not
	 *         terminated. Is <code>null</code> if no products are available
	 *         at the moment.
	 */
	 ...
	 * processes waiting in the queue. Note, that as long as the lifeCycles of
	 * the products (SimProcesses) leaving the Entrepot are not terminated they
	 * will automatically be activated after the current SimProcess! If no
	 * products complying with the given condition are available at the moment
	 ...
	 * 
	 * @return desmoj.SimProcess[] : All the products which are in compliance
	 ...
	 *         they are not terminated. Is <code>null</code> if no such
	 *         products are available at the moment.
	 * @param condition
	...
	public SimProcess[] removeAllProducts(Condition condition) {
		...
		// moment
		int n = (int) getAvailComplyWith(condition);
	...
	/**
	 * Removes (and returns) one product (SimProcess) from the Entrepot. Note,
	 * that as long as the lifeCycle of the product (SimProcess) leaving the
	 * Entrepot is not terminated it will automatically be activated after the
	 * current SimProcess! If no products are available at the moment the
	 * requesting SimProcess will be enqueued in the wait queue until a product
	 * becomes available. In case the capacity limit of the wait queue is
	 * reached the current SimProcess will be rejected and not get any product (
	 * <code>null</code> will be returned). The product will be retrieved in a
	 * kind of FIFO order, that means the product which arrived first in the
	 * Entrepot is the first to be removed.
	 * 
	 * @return desmoj.SimProcess : The product which is removed from the
	 *         Entrepot. The lifeCycles of this SimProcess will be activated as
	 *         long as he is not terminated. Is <code>null</code> if the
	 ...
	 * waiting in the queue already. Note that as long as the lifeCycle of the
	 * products (SimProcesses) leaving the Entrepot is not terminated they will
	 * automatically be activated after the current SimProcess! To use this
	 * method one must know which product(s) are in the Entrepot. To check which
	 * products are contained use methods like <code>products()</code> or
	 ...
	 * the Entrepot. Note, that as long as the lifeCycles of the products
	 * (SimProcesses) leaving the Entrepot are not terminated they will
	 * automatically be activated after the current SimProcess! If not enough
	 ...
	 * first to be removed.
	 * 
	 * @return desmoj.SimProcess[] : All the products which are removed from the
	 *         Entrepot. The lifeCycles of their SimProcesses will be activated
	 *         as long as they are not terminated. Is <code>null</code> if the
	 *         capacity limit of the wait queue is reached and therefore the
	 *         SimProcess is refused to be serviced or another failure has
	 *         occured.
	 * @param n
	...
	/**
	 * Removes (and returns) a certain number of products (SimProcesses) from
	 * the Entrepot which are in compliance with the given condition. Note, that
	 ...
	 * given condition use the method <code>getAvailComplyWith()</code>.
	 * 
	 * @return desmoj.SimProcess[] : All the products which are in compliance
	 *         with the given condition and are removed from the Entrepot. The
	 *         lifeCycles of their SimProcesses will be activated as long as
	 ...
	 *            given condition) to be removed from the Entrepot.
	 * @param condition
	 *            desmoj.Condition : The condition the products to be removed
	 *            must comply with.
	 */
	public SimProcess[] removeProducts(int n, Condition condition) {
	...
	/**
	 * To reset the statistics of this Entrepot. The number of available
	 * products at this moment and the processes waiting in the queue are not
	 * changed. But all statistic counters are reset. The
	...
	/**
	 * Stores an array of SimProcesses as products in the Entrepot. Make sure
	...
	/**
	 * Updates the statistics every time a product (SimProcess) is stored in the
	...
	protected void updateStatistics(long n) {
		...
		{
			soldProducts += java.lang.Math.abs(n);
			customers++;
		}
	}
}