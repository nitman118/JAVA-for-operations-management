...
/**
 * A class to run a batch of simulation experiments from the console. The
 * batches must be specified in an XML file. The XML file must start with a
 * <batch>tag. In the top level tag it is possible to specify model and
 * experiment settings valid for every run using the <model>and <exp>tag known
			...
			for (int i = 0; i < runs.getLength(); i++) {
				...
				if (nextDesc.getNodeName().equals("run")) {
					...
					try {
						...
						e.printStackTrace();
						throw new java.lang.RuntimeException("Cannot access experiment runner thread.");
					}
