 ...
 * A EntityScheduleEntry is a time region (e.g. every workingday from 8:00 until 16:00)
 * All EntityScheduleEntry's have the same timezone, stored in EntitySchedule.
 * 
 * EntitySchedule is a SimProcess. With setProcess is a process specified.
 * The lifecycle of EntitySchedule will activate the with setProcess specified process
 * at begin and end of each schedule entry.
 * 
...
public class EntitySchedule<E extends SimProcess> extends SimProcess{
	...
	public EntitySchedule(Model owner, String name, E process){
		...
		if(process != null) this.processList.add(process);
	 ...
	 * @param date
	 * @return		Values des ersten Entry, der date enthaelt.
	 * 				null wenn kein Entry gefunden
	 */
	public Object[] isInSchedule(TimeInstant time){
		...
		while(e.hasMoreElements()){
			...
			if(entry.isInEntry(time)) return entry.getValues();
		}
		return null;
	}
	 ...
	 * Die Endzeit gehoert nicht zum ScheduleEntry
	 * @return		Capacity des ersten Entry, der date enthaelt.
	 * 				null wenn kein Entry gefunden
	 */
	...
	public TimeInstant getBeginOfNextEntry(TimeInstant now){
		...
		for(int i=0; i<this.entryList.size(); i++){
			...
			if(nextBegin != null && TimeInstant.isBefore(nextBegin, min)) min = nextBegin;
		}
		if(min.equals(big)) min = null;
		return min;
	}
	 ...
	 * Angenommen in jedem schedule entity steht eine unterschiedliche Anzahl der 
	 * Resource e zur Verfuegung und diese Anzahl ist in der Komponente indes des 
	 * values-array abgespeichert.
	 * Die Namen der Resourcen Entities enthalten am Ende eine laufende Nr.
	 * Diese Methode prueft, ob diese Nr <= der in dem aktuellen Schedule 
	 * zulaessigen Anzahl ist.  
	 * @param e
	 * @param index
	 * @return
	 */
	public boolean checkAvailability(Entity e, int index){
		...
		if(this.isInSchedule() != null){
			...
			if( tmp instanceof Double)verfuegbar = Math.round((Double) tmp);
			else if( tmp instanceof Long)verfuegbar = (Long) tmp;
			else{
				throw (new desmoj.core.exception.SimAbortedException(
						new desmoj.core.report.ErrorMessage(
								...
								"Class : EntitySchedule  checkAvailability : value["+index+"] is not of type Double or Long",
	...
	private void processActivate(){
		if(this.processActivate){
			...
			while(en.hasMoreElements()){
				...
				if(! process.isScheduled()) process.activate(new TimeSpan(0));
			}
		}
	}
	
	private void processPassiviate(){
