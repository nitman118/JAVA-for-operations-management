 ...
 * In every step you can add a set of process-entities and a set of resource-entities.
 * This process-entities are processed together and use for this the resources 
 * of the associated resource set. 
 * 
 * There are two different Types of processes:
 * In a non abstract process every involved process- and resource-entity is stored explicitly.
 * In an abstract process only the involved process-entities are stored explicitly. Here we 
 * assume, all resource-entities are identical. Only the total no of available 
 * and no of used resources are stored.
 * 
 * NEW Version 
 * 
...
public class ProcessNew implements Basic {
	...
	*/
	private String 				id;
	private String 				name;
	private boolean				abstractResources;
	...
	public ProcessNew(Model model, String praefix, String id, String resourceType, int resourceTotal, String listId){
		...
		if(this.id != null) model.getProcessNewes().add(this);
	}
	...
	public void setResourceType(String type) throws ModelException{
		if(this.abstractResources)
			...
			throw new desmoj.extensions.visualization2d.engine.model.ModelException("a resourceType of an non-abstract resource is not allowed.");
	}
	...
	public void setResourceTotal(int total) throws ModelException{
		if(this.abstractResources)
			...
			throw new desmoj.extensions.visualization2d.engine.model.ModelException("resourceTotal of an non-abstract resource is not allowed.");
	}
	 ...
	 * Has no information when process isn't abstract.
	 * @return
	 */
	 ...
	 * @param index
	 * @return	no. of resources in resource-set with index
	 * 			-1 when no resource-set with index
	 */
	...
	/**
	 * adds set of process- and resource-entities to process. An additional entry is created.
	 * Only for non abstract processes
		...
		if(! this.abstractResources){
			...
			for(int i=0; i<resourceEntityIds.length; i++){
				...
				if(entity != null && entity.isFree()){
					...
					throw new ModelException("resourceEntity: "+resourceEntityIds[i]+" does not exist or is in use");
				}
			}
			this.entries.add(pe);
			if(this.grafic != null) ((ProcessNewGrafic)this.grafic).update();
			//this.printEntries();
		}
	}
	
	/**
	 * adds set of process- and no. of resources to process. An additional entry is created.
	 * Only for abstract processes
	 * @param processEntityIds		array of process-entity-id's
	 ...
	 * @param time		simulation time
	 * @return			out[0] array of removed process-id's
	 * 					out[1] array of removed resource-id's (only for non-abstract processes)
	 * 					this arrays are empty, when no entry to remove found.
	...
	public void printEntries(){
		...
		if(this.isAbstractResource()){
			...
			System.out.println(prefix+"Resources free-used-total   :"+this.getResourceFree()+"-"+this.getResourceUsed()+"-"+this.getResourceTotal());
		}
		for(int i=0; i<this.getEntriesAnz(); i++){
	...
	public Grafic createGrafic(String viewId, int x, int y, String defaultEntityTypeId, 
		...
		this.grafic =  new ProcessNewGrafic(this, viewId, new Point(x, y), defaultEntityTypeId, anzVisible, horizontal, showResources, deltaSize);
		return this.grafic;
	}
