...
public class WaitingQueueGrafic extends JPanel implements Grafic{
	...
	private JScrollPane 	masterWaitPane, slaveWaitPane, cooperationPane;
	private JPanel 			masterWaitFlow, masterWaitPanel;
	private JPanel			slaveWaitFlow, slaveWaitPanel;
	private JPanel			cooperationFlow, cooperationPanel;
	private JLabel 			masterWaitLength, slaveWaitLength;
	private JLabel			noOfCooperations;
	 ...
	 * @return 	smallest rectangle when an WaitingQueueGrafic exist,
	 * 			null otherwise.
	 */
	...
	public WaitingQueueGrafic(WaitingQueue waitingQueue, String viewId, Point pointExtern, 
		...
		if(waitingQueue.getName() != null) 	this.code = this.waitingQueue.getName();
		else 							this.code = "id: "+this.waitingQueue.getId();
		...
		JPanel cornerM = new JPanel(); cornerM.setLayout(new GridLayout(1,1)); cornerM.setBackground(Grafic.COLOR_BACKGROUND);
		JPanel cornerS = new JPanel(); cornerS.setLayout(new GridLayout(1,1)); cornerS.setBackground(Grafic.COLOR_BACKGROUND);
		JPanel cornerC = new JPanel(); cornerS.setLayout(new GridLayout(1,1)); cornerC.setBackground(Grafic.COLOR_BACKGROUND);
		...
		if(this.horizontal){
			...
			this.slaveWaitPane = new JScrollPane(this.slaveWaitFlow, JScrollPane.VERTICAL_SCROLLBAR_NEVER, JScrollPane.HORIZONTAL_SCROLLBAR_AS_NEEDED);
			...
			this.masterWaitPane = new JScrollPane(this.masterWaitFlow, JScrollPane.VERTICAL_SCROLLBAR_NEVER, JScrollPane.HORIZONTAL_SCROLLBAR_AS_NEEDED);
			...
			this.cooperationPane = new JScrollPane(this.cooperationFlow, JScrollPane.VERTICAL_SCROLLBAR_NEVER, JScrollPane.HORIZONTAL_SCROLLBAR_AS_NEEDED);
			...
			this.slaveWaitPane = new JScrollPane(this.slaveWaitFlow, JScrollPane.VERTICAL_SCROLLBAR_AS_NEEDED, JScrollPane.HORIZONTAL_SCROLLBAR_NEVER);
			...
			this.masterWaitPane = new JScrollPane(this.masterWaitFlow, JScrollPane.VERTICAL_SCROLLBAR_AS_NEEDED, JScrollPane.HORIZONTAL_SCROLLBAR_NEVER);
			...
			this.cooperationPane = new JScrollPane(this.cooperationFlow, JScrollPane.VERTICAL_SCROLLBAR_AS_NEEDED, JScrollPane.HORIZONTAL_SCROLLBAR_NEVER);
	 ...
	 * Will be called from Resource.provide, Resource.takeProcess and Resource.takeBack.
