...
public class StockGrafic extends JPanel implements Grafic{
	...
	private JScrollPane 	consumerPane, producerPane;
	private JPanel 			consumerFlow, consumerPanel;
	private JPanel			producerFlow, producerPanel;
	private JPanel			stockPanel;
	private JLabel 			consumerLength, producerLength;
	 ...
	 * @return 	smallest rectangle when an StockGrafic exist,
	 * 			null otherwise.
	 */
	...
	public StockGrafic(Stock stock, String viewId, Point pointExtern, 
		...
		if(stock.getName() != null) 	this.code = this.stock.getName();
		else 							this.code = "id: "+this.stock.getId();
		...
		JPanel cornerC = new JPanel(); cornerC.setLayout(new GridLayout(1,1)); cornerC.setBackground(Grafic.COLOR_BACKGROUND);
		JPanel cornerP = new JPanel(); cornerP.setLayout(new GridLayout(1,1)); cornerP.setBackground(Grafic.COLOR_BACKGROUND);
		...
		if(this.horizontal){
			...
			legendeConsumer.add(legendeConsumer1Label); legendeConsumer.add(legendeConsumer2Label);
			...
			legendeProducer.add(legendeProducer1Label); legendeProducer.add(legendeProducer2Label);
			
			this.consumerPane = new JScrollPane(this.consumerFlow, JScrollPane.VERTICAL_SCROLLBAR_NEVER, JScrollPane.HORIZONTAL_SCROLLBAR_AS_NEEDED);
			...
			this.producerPane = new JScrollPane(this.producerFlow, JScrollPane.VERTICAL_SCROLLBAR_NEVER, JScrollPane.HORIZONTAL_SCROLLBAR_AS_NEEDED);
			...
			this.consumerPane = new JScrollPane(this.consumerFlow, JScrollPane.VERTICAL_SCROLLBAR_AS_NEEDED, JScrollPane.HORIZONTAL_SCROLLBAR_NEVER);
			...
			this.producerPane = new JScrollPane(this.producerFlow, JScrollPane.VERTICAL_SCROLLBAR_AS_NEEDED, JScrollPane.HORIZONTAL_SCROLLBAR_NEVER);
	...
	private void updateStockPanel(){
		...
		this.stockLwb.setText("LWB: "+Long.toString(stock.getLwb()));
		this.stockUpb.setText("UPB: "+Long.toString(stock.getUpb()));
