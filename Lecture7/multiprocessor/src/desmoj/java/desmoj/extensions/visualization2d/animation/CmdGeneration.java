...
public class CmdGeneration extends WriteCmds{
	 ...
	 * Must be between experiment constructor 
	 * and model-experiment connect.<br>
	 * Attention: It makes no sence to define a TimeInstant 
	 * before experiment constructor.
	 * 
	...
	public void setStartStopTime(TimeInstant begin, TimeInstant end, TimeZone timezone){
		...
		if(this.experimentBegin == null) throw new SimulationException("ExperimentBegin-Parameter is null");
		this.experimentEnd		= end;
		this.timezone			= timezone;
		if(this.timezone == null) this.timezone = DEFAULT_PREFERRED_TIMEZONE;
	}
	 ...
	 * The commands are also logged in logFileName from constructor.
	 * For semantic check the whole animation environment is build in heap. 
	 * This consumes a lot of time and heap space. 
	 * Normally the CheckAndLog Method is switched off.
	 */
	 ...
	 * is flushed in cmdFile. Default is 1000
	 * A small value reduces the uses buffer space and may be increases the running time. 
	 * @param flushSeq
	 */
	 ...
	 * When the buffer has more than this.flushSeq comands,
	 * the buffer is flushed in cmdFile
	 ...
	 * For semantic check the whole animation environment is build in heap. 
	 * This consumes a lot of time and heap space. 
	 * This Method works only when its switched on with setCheckAndLog() Method.
	 * Normally the CheckAndLog Method is switched off.
	 * @param c		Command to write
	 */
	public void checkAndLog(Command c){
		if(this.checkAndLogOn) super.checkAndLog(c);
	}
	 ...
	 * This id is used as an internal id by building animation objects.
	 * @param name
	 * @return
	 */
