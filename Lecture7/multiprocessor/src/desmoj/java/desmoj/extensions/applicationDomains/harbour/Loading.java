 ...
 * Loading is used with the <code>WaitQueue</code> construct, where a
 ...
 * carried out together is described in the method <code>cooperation</code>,
...
public class Loading extends ProcessCoop {
	 ...
	 * slave (<code>Truck</code> or <code>InternalTransporter</code>) in a
	 * cooperate process. The time it takes to load the containers (goods) is
	 * determined by the specified loadTimeStream.
	 * 
	 * @param owner
	 *            desmoj.Model : The model this Loading is associated to.
	 * @param name
	...
	/**
	 * This method describes the loading process carried out by the master
	 * process (some kind of <code>InternalTransporter</code> or
	 * <code>Crane</code>). In this simple case only the time it takes to
	 * load the containers (goods) is taken into consideration. If the user
	 * building the model has to implement a more complex behavior, he has to
	 * overwrite this method in a subclass. The time it takes to load the
	 * containers (goods) is obtained from the method
	 * <code>getLoadTimeSample()</code>.
	 * 
	 * @param master
	 *            SimProcess : The master process which really loads the
	 *            cooperation. Should be a subclass of
	 *            <code>InternalTransporter</code> or <code>Crane</code>.
	 * @param slave
	 *            SimProcess : The slave process which is lead through the
	 *            cooperation by the master. Should be a subclass of
	...
	protected void cooperation(SimProcess master, SimProcess slave) {
		...
		if ((master instanceof InternalTransporter) && (slave instanceof Truck)) {
			...
			if ((master instanceof desmoj.extensions.applicationDomains.harbour.Crane)
				...
				desmoj.extensions.applicationDomains.harbour.Crane c = (Crane) master;
	 ...
	 * load the containers (goods) with the transporter or crane. The time is
	 * taken from the given random number stream loadTimeStream.
	 * 
	 * @return desmoj.SimTime : The time it takes to load the containers (goods)
	 *         with the transporter/crane.
	 */
	...
	/**
	 * Sets the loadTimeStream to a new <code>RealDist</code> random number
	 * stream.
	 * 
	 * @param loadTimeStream
