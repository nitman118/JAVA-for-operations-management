 ...
 * carries goods (containers) around in a container terminal. It has a certain
 * loaded and empty speed when he drives in a terminal and is loaded or empty.
 * He has also a certain transporter control which he gets his <code>Job</code>
 ...
 * that one is. Its <code>lifeCycle()</code> is already implemented and
 ...
 * InternalTransporter if he does an emport or export job.
 * 
 * @see Transporter
 * @see T_Control
 * @see Job
...
public abstract class InternalTransporter extends Transporter {
	 ...
	 * (goods) in a container terminal. Implement its
	 ...
	 * value one.
	 * 
	 * @see Transporter It has a certain loaded and empty speed when he drives
	 ...
	 *            associated to.
	 * @param name
	 *            java.lang.String : The name of this InternalTransporter.
	 * @param capac
	 *            int : The maximum number of goods this InternalTransporter can
	 *            carry around.
	 * @param speedLoad
	 *            double : The loaded speed of this InternalTransporter.
	 * @param speedEmpty
	 *            double : The empty speed of this InternalTransporter.
	 * @param ts
	 *            <code>T_Control</code>: The transporter control wchich this
	 ...
	 * 
	 * @param c
	 *            <code>Crane</code>: The new assigned crane of this
	 *            InternalTransporter.
	 */
	public void setCrane(desmoj.extensions.applicationDomains.harbour.Crane c) {
	 ...
	 * transporter if he is loaded (has some goods). In this simple case only
	 * the time it takes to drive the distance with his loaded speed. The number
	 * and time of loaded drives were counted in this method.
	 * 
	 * @param distance
	 *            double : The distance to drive
	 */
	public void driveLoad(double distance) {

		// define the needed time to drive
		SimTime t = new SimTime(distance / this.speedLoad);

		// trace with the note that the internal transporter is driving loaded
		// now
		if (currentlySendTraceNotes()) {
			sendTraceNote("drives loaded for "
					+ t.toString(getModel().getExperiment().getTimeFloats())
					+ " until "
					+ SimTime.add(currentTime(), t).toString(
		...
		// transporter
		this.sumLoadDrives = this.sumLoadDrives + t.getTimeValue();
		...
		this.ts.getTS().addSumTimeLoadedDrives(t.getTimeValue());
	...
	/**
	 * This method describes the driving the certain distance of the internal
	 * transporter if he is empty (has no goods). In this simple case only the
	 * time it takes to drive the distance with his empty speed. The number and
	 * time of empty drives were counted in this method.
	 * 
	 * @param distance
	 *            double : The distance to drive
	 */
	public void driveEmpty(double distance) {
		...
		SimTime t = new SimTime(distance / this.speedEmpty);
		...
		// transporter
		sumEmptyDrives = sumEmptyDrives + t.getTimeValue();
		...
		this.ts.getTS().addSumTimeEmptyDrives(t.getTimeValue());
		this.ts.getTS().addSumDistanceEmptyDrives(distance);
	}

	/**
	 * Sets the empty speed of this InternalTransporter to a new value. The new
	 * value must not be zero or negative.
	 * 
	 * @param s
	 *            double : The new empty speed of this InternalTransporter.
	 */
	...
	/**
	 * Sets the loaded speed of this InternalTransporter to a new value. The new
	 * value must not be zero or negative.
	 * 
	 * @param s
	 *            double : The new loaded speed of this InternalTransporter.
	 */
	 ...
	 * 
	 * @param j
	 *            <code>Job</code>: The new job of this InternalTransporter.
	 */
	...
	/**
	 * Sets the current capacity of this InternalTransporter to a new value. The
	 * new value must not be negative.
	 * 
	 * @param c
	 *            int : The new current capacity of this InternalTransporter.
	 */
	...
	/**
	 * Returns the current capacity of this InternalTransporter. That is the
	 * number of goods (containers) he carries at the moment.
	 * 
	 ...
	 * 
	 * @param j
	 *            <code>Job</code>: The import job of this
	 *            InternalTransporter.
	 */
	public abstract void importJobCycle(Job j);
	 ...
	 * 
	 * @param j
	 *            <code>Job</code>: The export job of this
	 *            InternalTransporter.
	 */
	public abstract void exportJobCycle(Job j);

	/**
	 * This method implements the internal transporter specific behaviour. This
	 * behavior is always the same therefore there is no need to implement that
	 * in a subclass of InternalTransporter. This method starts after an
	 * internal transporter has been created and activated by the scheduler.
	 */
	 ...
	 * done.
	 * 
	 * @return long : The whole time of empty drives of this
	 *         InternalTransporter.
	 */
	 ...
	 * done.
	 * 
	 * @return long : The whole time of loaded drives of this
	 *         InternalTransporter.
	 */
	...
	/**
	 * This method describes the loading of the internal transporter. In this
	 * simple case only the time it takes to load a container/good on itself.
	 * 
	 * @param time
	 *            SimTime : The needed time to load.
	 */
	public void pickUp(SimTime time) {

		// trace with the note that the internal transporter is picking up now
		if (currentlySendTraceNotes()) {
			sendTraceNote("picks up for "
					+ time.toString(getModel().getExperiment().getTimeFloats())
					+ " until "
					+ SimTime.add(currentTime(), time).toString(
							getModel().getExperiment().getTimeFloats()));
		}
	...
	/**
	 * This method describes the unloading of the internal transporter. In this
	 * simple case only the time it takes to unload a good/container from
	 * itself.
	 * 
	 * @param time
	 *            SimTime : The needed time to unload.
	 */
	public void pickDown(SimTime time) {
