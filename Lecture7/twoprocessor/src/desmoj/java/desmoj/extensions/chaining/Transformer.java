 ...
 * entity. The user has to override the method transformEntity() to define the
 * entitiy which the transformer should emit . The Tranformer inherits from the
 * Splitter and reduces its complexity for the new purpose. Within the
 * Transformer executes the same behaviour like the Splitter.
 * 
 * @param <EIn>
 *            the entitiy which comes in the Tranformer
 * @param <EEx>
...
public abstract class Transformer<EIn extends Entity, EEx extends Entity>
	 ...
	 * the free service capacity queue. If a given int is lower than zero the
	 * Source assumes that there is no resriction. This applies to
	 ...
	 * @param transportTime
	 *            the time the station needs to transport the entitiy to the
	 *            next station
	 * @param owner
	 *            the model owner
	 * @param name
	...
	/**
	 * This Method has to be overridden to create the output entity of the
	 * Transformer. The method gives the user the possibility to use the
	 * originalEntity in the creation of the transormEntity
	 * 
	 * @param originalEntity
	 *            a reference to the original entity
