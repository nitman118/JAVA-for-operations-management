...
import desmoj.extensions.applicationDomains.production.RestockProcessQT;
 ...
 *
 */
public class RestockProcessQTAnimation extends RestockProcessAnimation{

	public RestockProcessQTAnimation(RestockProcessQT restock) {
		this.restock			= restock;
		this.check(restock);
	}
	
	private void check(RestockProcessQT restock){
		if (! (restock.getModel() instanceof ModelAnimation)) 
			throw new SimulationException("For animation, RestockProcessQT must be initialized with a model of type ModelAnimation");
	}
