...
public class RouteGrafic extends Object implements Grafic{
	...
	private Point[] 			pointsExtern, pointsIntern;
	 ...
	 * @return 	smallest rectangle when an routeGrafic exist,
	 * 			null otherwise
	 */
	...
	public RouteGrafic(Route route, Point[] pointsExtern , long time) {
		...
		this.transform(time);
		if(this.route.getName() != null) 	this.code = this.route.getName();
		else 							this.code = "id: "+this.route.getId();
		...
		for(int i=1; i<this.pointsExtern.length; i++){
			this.middleIndex = i;
			if(this.routeRatio[i]>0.5)break;
		}
	...
	/**
	 * only for internal use, called by constructor
	 * determine viewId by view of source and sink station
	 * @param route
	 */
	...
	class RouteStaticGrafic extends Object{
		...
		/**
		 * Paint the line of this route.
		 * This method will be called from 
		 * RouteStaticGraficComponent.paintComponent()
		 * @param g
		 */
		public void paintComponent(Graphics g){
			...
			if(middleX != 0) 	angle 	= Math.atan((double)middleY/(double)middleX);
			//System.out.println("angle: "+angle+" mx:"+middleX+" my:"+middleY);
			if((route.getShow() & Route.SHOW_DIRECTION) != 0){
				if(middleX > 0) 		text = text + " >";
				else if(middleX < 0)	text = "< " + text;
				else if(middleY > 0)	text = "< " + text;
				else					text = text + " >";
			}
			...
			g2.fillRect(middleX-fm.stringWidth(text)/2, middleY-fm.getAscent()/2, fm.stringWidth(text), fm.getAscent());
			g2.setColor(route.getColor());
			...
			if(! route.getModel().getStations().exist(route.getSourceId())){
				...
				if(middleX >= 0){
					...
					g2.fillRect(pointsIntern[0].x, pointsIntern[0].y-fm.getAscent()/2, fm.stringWidth(text), fm.getAscent());
					g2.setColor(Color.red);
					g2.drawString(text, pointsIntern[0].x, pointsIntern[0].y+fm.getAscent()/2);
				}
				else{
					g2.setColor(Color.yellow);
					g2.fillRect(pointsIntern[0].x-fm.stringWidth(text), pointsIntern[0].y-fm.getAscent()/2, fm.stringWidth(text), fm.getAscent());
					g2.setColor(Color.red);
					g2.drawString(text, pointsIntern[0].x-fm.stringWidth(text), pointsIntern[0].y+fm.getAscent()/2);
				}
				g2.setTransform(t);
			}
			...
			if(! route.getModel().getStations().exist(route.getSinkId())){
				...
				if(middleX >= 0){
					g2.setColor(Color.yellow);
					g2.fillRect(pointsIntern[pointsIntern.length-1].x-fm.stringWidth(text), pointsIntern[pointsIntern.length-1].y-fm.getAscent()/2, fm.stringWidth(text), fm.getAscent());
					g2.setColor(Color.red);
					g2.drawString(text, pointsIntern[pointsIntern.length-1].x-fm.stringWidth(text), pointsIntern[pointsIntern.length-1].y+fm.getAscent()/2);
				}
				else{
					g2.setColor(Color.yellow);
					g2.fillRect(pointsIntern[pointsIntern.length-1].x, pointsIntern[pointsIntern.length-1].y-fm.getAscent()/2, fm.stringWidth(text), fm.getAscent());
					g2.setColor(Color.red);
					g2.drawString(text, pointsIntern[pointsIntern.length-1].x, pointsIntern[pointsIntern.length-1].y+fm.getAscent()/2);
				}
				g2.setTransform(t);
			}
		}
	...
	class RouteDynamicGrafic extends Object implements MouseListener{
		...
		/**
		 * Paint the entities in this route.
		 * This method will be called from 
		 * RouteDynamicGraficComponent.paintComponent()
		 * @param g
		 */
		public void paintComponent(Graphics g){
			Graphics2D g2 = (Graphics2D) g;
			...
			for(int i=0; i<content.length; i++){
				...
				for(int j=1; j<routeRatio.length; j++){
					if(routeRatio[j] > relDist.doubleValue()){
						...
						deltaX		= (double)(pointsIntern[j].x-pointsIntern[j-1].x);
						deltaY		= (double)(pointsIntern[j].y-pointsIntern[j-1].y);
				...
				if(Math.abs(relDist.doubleValue()-1.0)<eps) {
					...
					deltaX		= (double)(pointsIntern[pointsIntern.length-1].x-pointsIntern[pointsIntern.length-2].x);
					deltaY		= (double)(pointsIntern[pointsIntern.length-1].y-pointsIntern[pointsIntern.length-2].y);
					pos.setAngle(Math.atan(deltaY/deltaX));
					pos.setDirection((deltaX >= eps)||((Math.abs(deltaX)< eps)&&(deltaY < 0.0 )));
				}
				if(Math.abs(pos.getAngle()-Math.PI/2.0)< eps) pos.setAngle(-Math.PI/2.0);
				if(pos != null){
		...
		/**
		 * Check MouseEvents again the entities of this route.
		 * By success the event will dispached to the entity. 
		 * @param e MouseEvent 
		 */
