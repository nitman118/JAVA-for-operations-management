...
public class StockAnimation extends Stock{
	...
	private int				producerSortOrder;
	private int				consumerSortOrder;
	...
	public void createAnimation(Position pos, FormExt form,
		...
		if(this.showInAnimation){
			try {
				if(init)	c = Command.getCommandInit("createStock", this.cmdGen.getAnimationTime(simTime));
				else 		c = Command.getCommandTime("createStock", this.cmdGen.getAnimationTime(simTime));
	  ...
	  * @param noOfProducts
	  * @return			    true:  Consumer got the products
	  * 					false: Consumer is rejected,
	  * 							because the waiting queue of consumer is full
	  */
	 public boolean retrieve(long noOfProducts){
		SimProcess consumer = this.currentSimProcess();
		boolean out;
		TimeInstant simTime = this.getModel().presentTime();
		boolean	init	= this.cmdGen.isInitPhase();
		Command c;
		String[] retrieve = {consumer.getName(), Integer.toString(consumer.getPriority()), Long.toString(noOfProducts), this.consumerPriorityAttribute};
		if(this.showInAnimation){
			try {
				if(init)	c = Command.getCommandInit("setStock", this.cmdGen.getAnimationTime(simTime));
				else 		c = Command.getCommandTime("setStock", this.cmdGen.getAnimationTime(simTime));
				c.addParameter("StockId", this.id);
	  ...
	  * When the stock is full, he must wait in a queue of producer
	  * @param noOfProducts
	  * @return			    true:  Producer can store the products
	  * 					false: Producer is rejected, 
	  * 							because the waiting queue of producer is full
	  */
	 public boolean store(long noOfProducts){
		SimProcess producer = this.currentSimProcess();
		boolean out;
		TimeInstant simTime = this.getModel().presentTime();
		boolean	init	= this.cmdGen.isInitPhase();
		Command c;
		String[] store = {producer.getName(), Integer.toString(producer.getPriority()), Long.toString(noOfProducts), this.producerPriorityAttribute};
		if(this.showInAnimation){
			try {
				if(init)	c = Command.getCommandInit("setStock", this.cmdGen.getAnimationTime(simTime));
				else 		c = Command.getCommandTime("setStock", this.cmdGen.getAnimationTime(simTime));
