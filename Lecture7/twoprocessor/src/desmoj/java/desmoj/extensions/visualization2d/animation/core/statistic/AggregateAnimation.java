...
public class AggregateAnimation extends Aggregate{
	 ...
	 * @param showInReport			as in Aggregate
	 * @param showInTrace			as in Aggregate
	 * @param isResetResistant		as in Aggregate
	 */
	public AggregateAnimation(ModelAnimation owner, String name, 
			boolean showInReport, boolean showInTrace, boolean isResetResistant) {
		
		super(owner, name, showInReport, showInTrace, isResetResistant);
	...
	public void createAnimation(Position pos, Form form, boolean showInAnimation) {
		this.createAnimation(new TimeInstant(1.0), 0.0, 1.0, 10, pos, form, showInAnimation);
	}
	...
	public void createAnimation(TimeInstant highTime, double valMin, double valMax, 
		...
		if(this.showInAnimation){
			try {
				...
				c.addParameter("TypeIndex", Statistic.INDEX_Min_Max+"");
				...
				c.setRemark(this.getGeneratedBy(AggregateAnimation.class.getSimpleName()));
