 ...
 * The simulation-time-value is given by a long an can be interpreted 
 * in a java-typical way as a time-value.
 * 
 * Normally the time runs with a specified speed. It's can started, halted (pause), 
 * continued and stopped.
 * In a trace-mode the time can step from one value to the next.
 * 
...
public class SimulationTime {
	 ...
	 * time == Long.MIN_VALUE is written as "".
	...
	public SimulationTime(long start, long end, double speed, TimeZone timezone, Locale locale){
		...
		if(timezone != null) SimulationTime.timezone 	= timezone;
		if(locale != null) 	 SimulationTime.locale 		= locale;
	 ...
	 * In trace-mode it jumps to next step, set by setNextStep(). 
	 */
	public void cont(){
