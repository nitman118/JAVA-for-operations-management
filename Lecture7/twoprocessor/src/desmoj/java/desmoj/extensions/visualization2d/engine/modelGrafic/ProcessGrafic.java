...
public class ProcessGrafic extends JPanel implements Grafic, MouseListener{
	...
	private String					code;
	private JPanel 					resContPanel;
	private JPanel 					procContPanel;
	 ...
	 * @return 	smallest rectangle when an ProcessGrafic exist,
	 * 			null otherwise
	 */
	...
	public ProcessGrafic(desmoj.extensions.visualization2d.engine.model.Process process, String viewId, Point pointExtern, String defaultEntityTypeId) {
		...
		this.setBounds(this.pointIntern.x, this.pointIntern.y, Math.max(100,(d.width * Math.max(resEn.length, procEn.length))+ 30), zeilen * d.height + 26);
		...
		if(this.process.getResourceEntity().length > 0){
			JPanel resPanel = new JPanel(); resPanel.setLayout(new BorderLayout());
			resPanel.setOpaque(false);
			this.add(resPanel);
			JPanel resContPanel0 = new JPanel(); resContPanel0.setLayout(new FlowLayout(FlowLayout.CENTER,0,0));
			...
			JLabel r = new JLabel("R:"); r.setFont(Grafic.FONT_DEFAULT);
			resPanel.add(BorderLayout.WEST, r);
			resPanel.add(BorderLayout.CENTER, resContPanel0);
		}
		if(this.process.getProcessEntity().length > 0){
			JPanel procPanel = new JPanel(); procPanel.setLayout(new BorderLayout());
			procPanel.setOpaque(false);
			this.add(procPanel);
			JPanel procContPanel0 = new JPanel(); procContPanel0.setLayout(new FlowLayout(FlowLayout.CENTER,0,0));
			...
			JLabel e = new JLabel("E:"); e.setFont(Grafic.FONT_DEFAULT);
	 ...
	 * Will be called from Process.set and Process.unset
	 */
	public void update(){
		String[] resEn = this.process.getResourceEntity();
		String[] procEn = this.process.getProcessEntity();
	...
	private Dimension maxEntityDimension(){
		...
		for(int i=0; i<resEn.length; i++){
			if(! resEn[i].trim().equals("")){
				desmoj.extensions.visualization2d.engine.model.Entity en = process.getModel().getEntities().get(resEn[i]);
		...
		for(int i=0; i<procEn.length; i++){
			if(! procEn[i].trim().equals("")){
				desmoj.extensions.visualization2d.engine.model.Entity en = process.getModel().getEntities().get(procEn[i]);
				width 	= Math.max(width,  ((EntityGrafic)en.getGrafic()).getWidth());
				height 	= Math.max(height, ((EntityGrafic)en.getGrafic()).getHeight());
			}
		}
