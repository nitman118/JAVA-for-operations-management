...
public class DataListTallyReporter extends desmoj.core.report.TallyReporter {

	// ****** methods ******

	/**
	 * Constructor for a new DataListTallyReporter. Note that although any
	 * Reportable is accepted you should make sure that only subtypes of
	 * DataListTally are passed to this constructor. Otherwise the number of
	 ...
	 * 
	 * @param n
	 *            double : the double number to check
	 * @return true if the given double number is an integer number, false if
	 *         not.
	 */
	...
	/**
	 * Calculates the quartiles for the given list of data. This list have to be
	 * sorted to produce correct results. This method is called from the
	 * getEntries-Method to get all values needed for box-plots.
	 * 
	 * @param list
	 *            desmoj.core.statistic.DataListTally.DataList : The sorted list
	 *            to calculate the quartiles for.
	 * @return the quartiles from the given list.
	 */
	private static double[] getQuartiles(
			desmoj.core.statistic.DataListTally.DataList list) {
	...
	public String[] getEntries() {
		...
		if (source instanceof DataListTally) {
			...
			DataListTally.DataList list = dlt.getDataListSorted();
