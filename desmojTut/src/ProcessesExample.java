...
public class ProcessesExample extends Model {
    ...
    /**
     * Random number stream used to draw a service time for a truck. Describes the
     * time needed by the VC to fetch and load the container onto the truck. See
     * init() method for stream parameters.
     */
    private desmoj.core.dist.ContDistUniform serviceTime;
    /**
     * A waiting queue object is used to represent the parking area for the trucks.
     * Every time a truck arrives it is inserted into this queue (it parks) and will
     * be removed by the VC for service.
     *
    ...
    /**
     * A waiting queue object is used to represent the parking spot for the VC. If
     * there is no truck waiting for service the VC will return here and wait for
     * the next truck to come.
     *
     ...
     *
     * Creates a new ProcessesExample model via calling the constructor of the
     * superclass.
     *
     * @param owner
     *            the model this model is part of (set to null when there is no such
     *            model)
     * @param modelName
     *            this model's name
     * @param showInReport
     *            flag to indicate if this model shall produce output to the report
     *            file
     * @param showInTrace
     *            flag to indicate if this model shall produce output to the trace
     *            file
     */
     ...
     *
     * This method is used to place all events or processes on the internal event
     * list of the simulator which are necessary to start the simulation.
     *
     * In this case, the truck generator and the van carrier(s) have to be created
     * and activated.
     */
    public void doInitialSchedules() { // create and activate the van carrier(s)
        for (int i = 0; i < NUM_VC; i++) {
            VanCarrier vanCarrier = new VanCarrier(this, "Van Carrier", true);
            vanCarrier.activate();
            // Use TimeSpan to activate a process after a span of time relative to actual
            // simulation time,
    ...
    /**
     * Returns a sample of the random stream used to determine the time needed to
     * fetch the container for a truck from the storage area and the time the VC
     * needs to load it onto the truck.
     *
