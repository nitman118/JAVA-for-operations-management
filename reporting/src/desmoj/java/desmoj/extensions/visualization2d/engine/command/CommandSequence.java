...
public class CommandSequence {
	...
	/**
	 * liest Commandfile, und fuehrt Commands aus.
	 * Commands werden u.U. in Command.cmdBuffer zwischengespeichert.
	 * @param f			liest aus BufferedReader f
	 * @param init		Es werden nur init Commands ausgefuehrt.
	 * @param time		Es werden alle Commands zwischen dem letzten Aufruf 
	 * 					und bis einschliesslich time ausgefuehrt.
	...
	private long read(BufferedReader f, boolean init, long time) throws CommandException, ModelException, IOException{
		...
		Command last		= null;
		int		cmdCount	= 0;
		if(! this.readCmdBuffer.isEmpty()) last = this.readCmdBuffer.get(this.readCmdBuffer.size()-1);
		...
		if(init || last == null || (last.getTime() <= time)){
			...
			do{
				...
				do{
					...
					if(line == null){
						...
						line = 	Cmd.TIME_KEY+Cmd.KEY_VALUE_SEPARATOR+Long.MAX_VALUE+Cmd.PARAMETER_SEPARATOR+
								Cmd.INIT_KEY+Cmd.KEY_VALUE_SEPARATOR+Boolean.toString(false)+Cmd.PARAMETER_SEPARATOR+
				...
				do{
					...
					if(c == null)	System.out.println("c ist null");
				...
				if(last == null) System.out.println("last ist null");
				if(init) 	condition = last.isInit() && !last.getCmd().equals(Cmd.END_CMD);
				else		condition = (last.getTime() <= time) && !last.getCmd().equals(Cmd.END_CMD);
		...
		while(true){
			...
			if(init) 	condition = this.readCmdBuffer.get(0).isInit();
			else		condition = this.readCmdBuffer.get(0).getTime() <= time;
			if(! condition) break;
			...
			if(! command.syntaxCheck()) throw new CommandException("CommandFrame.read: SyntaxCheck failt", command.toString());
			command.execute(this.model);
			if(command.getCmd().equals(Cmd.END_CMD)) break;
