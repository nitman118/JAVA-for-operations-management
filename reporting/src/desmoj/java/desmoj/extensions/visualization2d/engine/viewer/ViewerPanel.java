 ...
 * Will be called by ViewerFrame and ViewerApplet
 * 
...
public class ViewerPanel extends JPanel{
	...
	public static InfoPane getInfoPane(){
		InfoPane out = null;
		if(viewer != null) out = viewer.infoPane;
		return out;
	}
	 ...
	 * @param locale				locale of this panel
	 */
	public ViewerPanel(URL cmdFile, URL simulationIconDir, URL snapshotDir, AppletContext appletContext, Locale locale) {	
		...
		if(this.dataFile != null && this.isApplication && this.snapShotDir == null){
			...
			if(projectData.isDirectory() && projectData.canWrite()){
				...
				if(! snapShot.exists()) snapShot.mkdir();
				try {
	 ...
	 * When this paths are null, the menue have no function
	 * used only for aplication version (not for applets)
	...
	/**
	 * this is fileReset doing:
	 * close simulationThread, close and reopen cmds-file, close and reopen log-file, 
	 * read and execute model-init-commands, show new model in gui,
	...
	private void fileReset() throws IOException {
		...
		if(this.simulationThread != null){
			...
			try {this.simulationThread.join();
		...
		if(startItem != null) 	startItem.setEnabled(true);
		if(stopItem != null) 	stopItem.setEnabled(false);
	...
	/**
	 * close simulationThread, close data-file,, close log-file, 
	 * reset model, remove model from gui
	 ...
	 * in der Mitte des selektierten View angezeigt wird. Analogon zum ChangeEventHandler
	 * beim Wechseln der ViewGrafic. 
	 * Wird mit SwingUtilities.invokeLater() als letzte Anweisung in der SwingEventQueue 
	 * ausgefuehrt.
	 * Voraussetzungen fuer den Aufruf:
	 * - Size des ViewerPanel muss feststehen (Nach dem ViewerFrame Konstruktor.
	 * Wird aufgerufen von:
	 * - nach der ViewerFrame Konstruktor, damit die Anzeige nach dem Neustart stimmt
	...
	/**
	 * Aktualisiert die angezeigte Simulationszeit, used by SimulationThread
	 * Fuehrt Commands bis zur aktuellen Simulationszeit aus (RunPhase)
	...
	protected void writeStatusMsg(){
		if(this.simulationTime.isRunning()) this.setStatusMessage(this.language.getString("StatusMsg_03"));
		else								this.setStatusMessage(this.language.getString("StatusMsg_04"));
	...
	public void updateSimulationTimeBounds(boolean valid){
		...
		if(valid && this.simulationTime != null){
			start	= SimulationTime.getTimeString(this.simulationTime.getSimulationStart(),SimulationTime.SHOW_DATE_TIME);
			end		= SimulationTime.getTimeString(this.simulationTime.getSimulationEnd(),SimulationTime.SHOW_DATE_TIME);
			time	= SimulationTime.getTimeString(this.simulationTime.getSimulationTime(),SimulationTime.SHOW_DAY_DATE_TIME_MILLIS_DST);
		}
	...
	public void setSimulationSpeedGUI(double speed){
		...
		String format = "#0.0";
		if(		0.1 <= speed 	&& speed < 1.0) 	format = "#0.0";
		else if(0.01 <= speed 	&& speed < 0.1) 	format = "#0.00";
		else if(0.001 <= speed 	&& speed < 0.01) 	format = "#0.000";
		else if(0.0001 <= speed && speed < 0.001) 	format = "#0.0000";
		else if( speed < 0.0001) 					format = "#0.00000";
	...
	private JPanel createHeaderPanel(){
		...
		zoomSlider = new JSlider(javax.swing.JSlider.HORIZONTAL,-20,20,0);
		...
		sb.setPreferredSize(new java.awt.Dimension(sb.getPreferredSize().width,5));
		...
		speedSlider = new JSlider(javax.swing.JSlider.HORIZONTAL,-7,10,0);
	 ...
	 * @param exitListener	ActionListener, which close the ViewerFrame
	 * 						when null, Data menuBar has no exitItem
	...
	private void updateMenuBar(){
		...
		if(this.menueBarIcons == null) this.menueBarIcons = new Vector<Component>();
	...
	class TimeFlow implements ActionListener{
		...
		public void actionPerformed(ActionEvent e){
			...
			switch(viewer.getTimeFlowMode()){
				case ViewerPanel.TimeFlowMode_CONTINIUM: 	defaultValue = selectionValues[0]; break;
				case ViewerPanel.TimeFlowMode_STEP_FLOW: 	defaultValue = selectionValues[1]; break;
				case ViewerPanel.TimeFlowMode_STEP_SINGLE: 	defaultValue = selectionValues[2]; break;
				default: 							defaultValue = selectionValues[1];
			...
			if(input == null) input = selectionValues[viewer.getTimeFlowMode()-1];
			if(input.equals(selectionValues[0])) viewer.setTimeFlowMode(ViewerPanel.TimeFlowMode_CONTINIUM);
			else if(input.equals(selectionValues[1])) viewer.setTimeFlowMode(ViewerPanel.TimeFlowMode_STEP_FLOW);
			else if(input.equals(selectionValues[2])) viewer.setTimeFlowMode(ViewerPanel.TimeFlowMode_STEP_SINGLE);
			else viewer.setTimeFlowMode(ViewerPanel.TimeFlowMode_CONTINIUM);
	...
	class SpeedMenu implements ActionListener{
		public void actionPerformed(ActionEvent e){
			...
			do{
				...
				ok		= true;
				try{
					if(input != null) speed = Double.parseDouble(input);
	...
	class SpeedSlider implements ChangeListener{
		public void stateChanged(ChangeEvent e){
			if(speedHandler){
				double speed = Math.exp((double)speedSlider.getValue());
				setSimulationSpeed(speed);
				setSimulationSpeedGUI(speed);
			}
		}
	...
	class SpeedField implements ActionListener{
		public void actionPerformed(ActionEvent e){
			if(speedHandler){
				...
				}catch(NumberFormatException ex){speed	= 1.0;	}
				speed = Math.abs(speed);
				setSimulationSpeed(speed);
				setSimulationSpeedGUI(speed);
			}
	...
	class ZoomMenu implements ActionListener{
		public void actionPerformed(ActionEvent e){
			...
			do{
				...
				ok		= true;
				try{
					if(input != null) zoom = Double.parseDouble(input);
				}catch(NumberFormatException ex){
					ok	= false;
				}
			}while(! ok);
			if(ok && input != null){
				...
				try{setSimulationZoom(zoom);
				}catch(desmoj.extensions.visualization2d.engine.modelGrafic.ModelGraficException e1){
					setStatusMessage(e1.getMessage());
					e1.printStackTrace(ViewerPanel.getLogWriter());
					ViewerPanel.getLogWriter().close();
					e1.printStackTrace();
	...
	class SnapShot implements ActionListener{
		...
		public void actionPerformed(ActionEvent e) {
			...
			if(snapShot != null && snapShot.isDirectory() && snapShot.canWrite()){
				...
				if(!isPause) simulationTime.pause();
				
				String time = SimulationTime.getTimeString(getSimulationTime(),SimulationTime.SHOW_DAY_DATE_TIME_MILLIS_DST);
				time = this.adjustFileName(time);
				// Subdirectory to store all images of a snapshot
				File directory = new File(snapShot, time);
				if(directory.isFile()){
					System.out.println("Error: "+directory.getAbsolutePath()+" is a file. A directory is expected.");
					if(!isPause) simulationTime.cont();
					return;
				}
	...
	class ZoomField implements ActionListener{
		public void actionPerformed(ActionEvent e){
			if(zoomHandler){
				...
				}catch(NumberFormatException ex){zoom	= 1.0;	}
				zoom = Math.abs(zoom);
				zoom = Math.max(0.00001, zoom);
	...
	class FileOpen implements ActionListener{
		public void actionPerformed(ActionEvent e){
			...
			if(viewer.defaultCmdPath != null){
		        ...
		        if(out != null) {
		        	if(out.getParent() != null)	viewer.defaultCmdPath = out.getParent();
		        	else 						viewer.defaultCmdPath = out.getPath();

		        	try{
		        		viewer.dataFile = out.toURI().toURL();
		        		viewer.fileOpen(viewer.dataFile);
		        		String outName = out.getParentFile().getName()+"/"+out.getName();
						viewer.setStatusMessage(language.getString("StatusMsg_10")+outName+" (choose Icondir before)");
		        	}catch(Exception e1){
						setStatusMessage(e1.getMessage());
						e1.printStackTrace(ViewerPanel.getLogWriter());
						ViewerPanel.getLogWriter().close();
	...
	class IconDirectory implements ActionListener{
		public void actionPerformed(ActionEvent e){
			...
			if(viewer.defaultIconPath != null){
		        ...
		        if(out != null) {
		        	if(out.getParent() != null)	viewer.defaultIconPath = out.getParent();
		        	else 						viewer.defaultIconPath = out.getPath();
	...
	class SnapShotDirectory implements ActionListener{
		public void actionPerformed(ActionEvent e){
			...
			if(viewer.defaultSnapShotPath != null){
		        ...
		        if(out != null) {
		        	if(out.getParent() != null)	viewer.defaultSnapShotPath = out.getParent();
		        	else 						viewer.defaultSnapShotPath = out.getPath();
