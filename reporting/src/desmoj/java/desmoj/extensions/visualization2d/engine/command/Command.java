 ...
 * Command is the connection to animation.model.
 * 
...
public class Command extends CommandFrame {
	 ...
	 * this method must include a case for every command-type.
	 */
	public void execute(Model model) throws CommandException, ModelException{
		...
		if(this.getCmd().equals("createModelBasisData")){
			...
			model.createEntityTyp(entityTypId[0], iconWidth[0], iconHeigth[0], posibleStates, posibleAttributes, show[0], this.isInit(), this.getTime());
			...
			String[] imageId			= this.getParameterType("ImageId").getValues();
			model.createBackgroundElement(backgroundElementId[0], name, text, topLeft, bottomRight, middle, size, foreground, background, level[0], imageId, this.isInit(), this.getTime());
			...
			String[] background			= this.getParameterType("Background").getValues();
			String[] imageId			= this.getParameterType("ImageId").getValues();
			model.setBackgroundElement(backgroundElementId[0], name, text, foreground, background, imageId, this.isInit(), this.getTime());
			...
			String[] level				= this.getParameterType("Level").getValues();
			model.createBackgroundLine(backgroundLineId[0], size, color, startPoint[0], addPoint, level[0], this.isInit(), this.getTime());
			...
			model.createEntity(entityId[0], name, entityTypeId[0], velocity, state[0], attribute, position, this.isInit(), this.getTime());
			...
			model.createList(listId[0], name, defaultEntityType[0], numberOfVisible[0], form[0], point[0], deltaSize, comment, this.isInit(), this.getTime());
			...
			model.setList(listId[0], addEntity, addEntityAfter, addEntityBefore, removeEntity, removeAll, this.isInit(), this.getTime());
			...
			model.createRoute(routeId[0], name, length[0], sourceStationId[0], sinkStationId[0], point, show[0], color, lineSize[0], this.isInit(), this.getTime());
			...
			String[] color				= this.getParameterType("Color").getValues();
			String[] lineSize			= this.getParameterType("LineSize").getValues();
			model.setRoute(routeId[0], addEntity, removeEntity, removeAll, show, color, lineSize, this.isInit(), this.getTime());
			...
			model.setProcess(processId[0], addResEntity, removeResEntity, addProcEntity, removeProcEntity, this.isInit(), this.getTime());
			...
			String[] removeEntity		= this.getParameterType("RemoveEntity").getValues();
			model.setProcessNew(processId[0], addProcEntity, addResEntity, addResAnz, removeEntity, this.isInit(), this.getTime());
			...
			model.setWaitQueue(waitQueueId[0], insertMaster, insertSlave, cooperationBegin, cooperationEnd, this.isInit(), this.getTime());
