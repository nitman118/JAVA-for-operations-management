 ...
 * or parts are assembled by a worker or machine. The list of parts needed for
 * this process are provided by means of a <code>PartsList</code>.
 * 
...
public class WorkStation extends QueueBased {
	...
	/**
	 * The number of different kinds of parts needed for processing the parts at
	 * this WorkStation. The number will be provided by the
	 * <code>PartsList</code>.
	 */
	...
	/**
	 * Array of counters for the slave SimProcesses which are refused to be
	 * enqueued in the different slave queues, because their capacity is full.
	 */
	protected long[] slavesRefused;

	/**
	 * The parts list containing all the parts with their respective quantity
	 * needed at this WorkStation to start the manufacturing (processing)
	 * process.
	 */
	 ...
	 * 
	 * @param owner
	 *            Model : The model this WorkStation is associated to.
	 * @param name
	 *            java.lang.String : The WorkStation's name
	 * @param partsList
	 ...
	 *            processes can be enqueued. Zero (0) means unlimited capacity.
	 * @param showInReport
	 *            boolean : Flag, if WorkStation should produce a report or not.
	 * @param showInTrace
	...
	/**
	 * Activates the <code>SimProcess</code>, given as a parameter of this
	 * method, as the next process. This process should be a master process
	 ...
	 * If all needed partes are available the processing process at this
	 * WorkStation can start.
	 * 
	 * @return boolean :<code>true</code>, if all parts listed in the parts
	 *         list are available at the moment. <code>false</code> otherwise
	 */
	 ...
	 * @param cond
	 *            Condition : The Condition <code>cond</code> is describing
	 ...
	 * in their queue and complying to the given condition. If there are no such
	 * slaves waiting at the moment <code>null</code> will be returned.
	 * 
	 * @return SimProcess[] : Returns all the slave processes of the given kind
	 *         that are complying to the given condition.
	 * @param kind
	 *            Class : The Class specifying of which kind the processes have
	 *            to be, we are looking for.
	 * @param cond
	 *            Condition : The Condition <code>cond</code> is describing
	...
	public SimProcess[] availSlaves(Class kind, Condition cond) {
		...
		if (index > -1) {
			...
			// condition
			java.util.Vector foundParts = new Vector();
	...
	/**
	 * Returns an array of all the slave processes needed to start the
	 * processing which are available and comply to the conditions given in the
	 * parameters. If no or not enough suitable slaves are available
	 * <code>null</code> will be returned.
	 * 
	 * @return desmoj.SimProcess[] : An array of all the slave processes which
	 *         are available and comply to the conditions given in the
	 *         parameters and are needed to start the processing. If not enough
	 *         of all of them are available <code>null</code> will be
	 *         returned.
	 * @param kinds
	 *            java.lang.Class[] : This array is specifying in conjunction
	 *            with the array conditions which slaves have to comply to which
	 *            conditions so the master will cooperate with them. Must be a
	 *            subset of the parts list.
	 * @param conditions
	 *            desmoj.Condition[] : This array is specifying in conjunction
	 *            with the array kinds which slaves have to comply to which
	 *            conditions so the master will cooperate with them.
	 */
	 ...
	 * 
	 * @return int : The number of different kinds of parts needed for
	 *         processing these parts at this WorkStation.
	 */
	...
	/**
	 * Returns the parts list of this WorkStation. That is the list listing all
	 * the different parts with their quantities needed to start the processing
	...
	/**
	 * Returns an array of <code>ProcessQueue</code> s where the waiting
	 * slaves are stored.
	 * 
	 ...
	 * at the given index as a String, so it can be displayed in the report. To
	 * get the index for the kind of part in question use the method
	 ...
	 * 
	 * @return long : The number of entities refused to be enqueued in the
	 *         slave's queue indicated by the index.
	 * @param index
	 ...
	 * 
	 * @return desmoj.core.TimeSpan : Longest waiting time of a process in the master
	 ...
	 * 
	 * @return desmoj.core.TimeInstant : The point of simulation time when the process
	 ...
	 * period since the last reset.
	 * 
	 * @return desmoj.core.TimeInstant : Point of time with minimum master queue length
	 *         since the last reset.
	 */
	 ...
	 * times.
	 * 
	 * @return desmoj.core.TimeSpan : The standard deviation for the master queue's processes
	 *         waiting times.
	 */
	...
	/**
	 * This method is to be called from a <code>SimProcess</code> which wants
	 ...
	 *         successfully in the master queue, <code>false</code> otherwise
	 ...
	 * should be a <code>Worker</code> or a <code>MachineProcess</code>. By
	 * using this method the master can determine some special slaves complying
	 * to certain conditions he would like to process. The <code>Class</code>
	 * array kinds must be a subset of the parts list and determines together
	 * with the <code>Condition</code> array (which must be of the same
	 ...
	 *            the master will cooperate with them.
	 */
	 ...
	 * queue and slave queues are reset. The mininum and maximum length of the
	 ...
	 * the last reset. If the time span since the last reset is smaller than the
	 * smallest distinguishable timespan epsilon, the current length of the
	 * slave queue will be returned.
	 * 
	 * @return double : The average length of the slave queue at the given index
	 *         since last reset or current length of the slave queue at the
	 *         given index if no distinguishable periode of time has passed.
	 * @param index
	 *            int : Indicating the kind of part (slave) for which we want to
	 *            know its average length. To get the index of a certain kind of
	 *            part (slave) use the method <code>getIndexOfKind()</code> of
	 *            the <code>PartsList</code>.
	 */
	...
	/**
	 * Returns the average waiting time of all processes who have exited the
	 * slave queue indicated by the index.. The returned value is valid for the
	 * time span since the last reset. Returns 0 (zero) if no process have
	 * exited this slave queue after the last reset.
	 * 
	 * @return TimeSpan : Average waiting time of all processes since last reset
	 *         or 0 if no process has exited this slave queue.
	 * @param index
	 *            int : Indicating the kind of part (slave) for which we want to
	...
	/**
	 * Sets the capacity of the given slave queue to a new value. But only if
	 * the new capacity is equal or larger than the current length of that slave
	 * queue! To get the index of the slave queue for which the capacity sould
	 * be changed use the method <code>getIndexOfKind()</code> of the
	 * <code>PartsList</code>.
	 * 
	 * @param index
	 *            int : The index indicating the slave queue for which the
	 *            capacity will be changed. To get that index one can use the
	 *            method <code>getIndexOfKind()</code> of the
	 *            <code>PartsList</code>.
	 * @param newCapacity
	 ...
	 * index. The sort order of the given slave queue can only be changed if it
	 * is empty! To get the index for the kind of part for which the queueing
	 * discipline should be changed use the method <code>getIndexOfKind()</code>
	 * of the <code>PartsList</code>. Please choose a constant from
	 ...
	 *            the method <code>getIndexOfKind()</code> of the
	 *            <code>PartsList</code>.
	 * @param sortOrder
	 *            int : determines the sort order of the underlying slave queues
	 ...
	 * 
	 * @return boolean : Is <code>true</code> if the indicated slave queue is
	 ...
	 * 
	 * @return long : The current length of the slave queue indicated by the
	 *         index.
	 * @param index
	 *            int : Indicating the kind of part (slave) for which we want to
	 ...
	 * since the last reset.
	 * 
	 * @return long : The maximum length of the indicated slave queue since the
	 *         last reset.
	 * @param index
	 *            int : Indicating the kind of part (slave) for which we want to
	 ...
	 *         slave queue had its maximum length since the last reset.
	 * @param index
	 *            int : Indicating the kind of part (slave) for which we want to
	...
	/**
	 * Returns the maximum duration in simulation time that an process has spent
	 * waiting inside the slave queue indicated by the index. The value is valid
	 * for the period since the last reset.
	 * 
	 * @return desmoj.core.TimeSpan : Longest waiting time of a process in the slave
	 *         queue since the last reset.
	 * @param index
	 *            int : Indicating the kind of part (slave) for which we want to
	...
	/**
	 * Returns the point of simulation time when the process with the maximum
	 * waiting time exited the slave queue indicated by the index. The value is
	 * valid for the period since the last reset.
	 * 
	 * @return desmoj.core.TimeInstant : The point of simulation time when the process
	 ...
	 * for the period since the last reset.
	 * 
	 * @return desmoj.core.TimeInstant : Point of time with minimum slave queue length
	 *         since the last reset.
	 * @param index
	 *            int : Indicating the kind of part (slave) for which we want to
	 ...
	 * by the index. The value is weighted over time.
	 * 
	 ...
	 * times. The slave queue is indicated by the index.
	 * 
	 ...
	 * master and enough other parts (or products) are available to start the
	 * manufacturing (processing) process. If the capacity limit of tis slave
	 * queue is reached, the process will not be enqueued and <code>false</code>
