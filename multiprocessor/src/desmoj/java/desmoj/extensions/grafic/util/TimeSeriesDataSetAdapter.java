...
public class TimeSeriesDataSetAdapter extends AbstractXYDataset
	 ...
	 * In the case timeSeries.getShowTimeSpansInReport() the data values are interpreted as
	 * a timespan in a appropriate time unit. 
	...
	public TimeSeriesDataSetAdapter(TimeSeries timeSeries, boolean allowMultipleValues){
		...
		if(isValid)	itemCount = (int)timeSeries.getObservations();
		else		itemCount = 0;
     ...
     * in the case timeSeries.getShowTimeSpansInReport()
     * a rangeTimeUnit is automatic determined
	 * This is the mayor unit, where the maximum time-series 
	 * data entry has more than threshold units.
	 * Example: threshold = 3
	...
	private TimeUnit chooseRangeTimeUnit(int threshold){
		...
		for(int i = 0; i<timeSeries.getObservations(); i++){
			...
			if(value < 0.0) {negativeTime = true; break;}
			TimeSpan time	= new TimeSpan(value);
			if(TimeSpan.isLonger(time, max)) max = time;
		}
		if(negativeTime) return out;
