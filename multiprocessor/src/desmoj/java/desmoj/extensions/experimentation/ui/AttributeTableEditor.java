...
public class AttributeTableEditor extends AbstractCellEditor implements
	...
	private AttribEditor defaultEditor;
	 ...
	 *            an attribute editor
	 * @param type
	 *            the class to edit with this editor
	 */
	...
	/**
	 * Registers the default editors. Might be extended in subclasses to
	 * register or exchange other specialized editors
	 */
	protected void registerEditors() {
		...
		registerEditor(e, desmoj.extensions.experimentation.util.Filename.class);
	}

	/**
	 * Creates the default editor. This is the editor, fields of a non specified
	 * type are edited with. Might be overridden in subclasses to exchange
	 * default editor.
	 * 
	...
	/**
	 * Implemented for interface TableCellEditor. Returns the editor for the
	 * given table cell value.
	 * 
	 * @param table
	 *            the current JTable
	 * @param value
	 *            the object to edit
	 * @param selected
	 *            flag indicating if the cell containing the object is selected
	 * @param row
	 *            row of table cell
	 * @param col
	...
	/**
	 * A generic table cell editor interface. Implement this interface for
	...
	public static class BooleanAttribEditor implements AttribEditor {
		...
		public Object getValue() {
			return (Boolean) c.getSelectedItem();
		}
	...
	/**
	 * A default text editor consisting of a single TextField. Can edit all data
	 * types with a constructor whose single argument is a String.
	 */
	public static class TextBasedEditor implements AttribEditor {
		...
		/**
		 * Returns null as an empty format. Subclasses can override this method
		 * to implement formats the edited text is validated against.
		 * 
		 * @return a new Format
		 */
		...
		/**
		 * Sets the text fields value to the given value. If the value is null
		 * the text field is initialized with an empty string
		 * 
		 * @param object
		 *            to init textfield with
		 */
		 ...
		 * edited class. Tries to create an object by invoking the current
		 * class' constructor with a single String argument. If this action
	...
	public static class IntegerNumberEditor extends TextBasedEditor {
		...
		/**
		 * Reads and returns integral number from the editor component. If it
		 * cannot be parsed correctly null is returned
		 * 
		 * @return a Number object whose actual subclass depends on the edited
		 *         object.
		 */
		public Object getValue() {
			try {
		...
		protected Format createFormat() {
			...
			((DecimalFormat) n).setMaximumFractionDigits(0);
			return n;
		}
	}
	...
	public static class FloatingPointNumberEditor extends TextBasedEditor {
		 ...
		 * If it cannot be parsed correctly null is returned
		 * 
		 * @return a Number object whose actual subclass depends on the edited
		 *         object.
		 */
