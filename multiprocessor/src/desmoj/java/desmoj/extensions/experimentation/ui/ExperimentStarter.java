...
// extends JFrame
	 ...
	 * 
	 * @param e
	 *            SimRunEvent: A SimRunEvent.
	 */
	public void clockAdvanced(SimRunEvent e) {
		SimTime currentTime = e.getSimTime();
		double timePercent = (currentTime.getTimeValue() / expRunner
				.getExperiment().getStopTime().getTimeValue()) * 100;
		experimentGUI.clockAdvanced(currentTime.toString(3), timePercent,
				startTime);
	}

	/**
	 * Creates a new experiment by loading a model and an experiment runner.
	...
	/**
	 * Inits a new experiment run from a given model and experiment runner
	 * class: model and experiment runner are initialized. Parameter tables are
	...
	/**
	 * Resets the currently active model by creating a new model instance of the
	 * current model class. Inits all model- and experiment parameters according
	 * to the current experiment runner.
	 */
	void resetModel() {
		try {
			if (docNode != null) {
	...
	/**
	 * Loads the DOM node representing the document currently open in the
	 * experiment launcher
	 */
	void loadDocNode() {
		try {
	...
	/**
	 * Saves the current Parameters into an XML File. To do so a DOM-Tree with
	 * the current Parameters is constructed. This tree will be serialized by
	 * use of XML_Util.serializeDocument and saved into the file selected via
	 ...
	 * 
	 * @param a
	 *            DOM node representing an xml batch file
	 */
	...
	/**
	 * Stops the current experiment (forever). Called when STOP button is
	 * pressed
	 */
	void stopExperiment() {
	...
	/**
	 * Pauses and resumes the currentExperiment (called when PAUSE button is
	 * switched).
	 */
	...
	/**
	 * Called when experiment is started or resumed. Implemented for
	 * ExperimentListener
	 */
