...
/**
 * A class representing a parametrized simulation run. The parameters can be
 * read from an XML file. The XML file must start with a <run>tag. This tag
 * contains model and experiment parameter specifications (tags <model>and
...
public class Run {
	...
	/**
	 * Creates a new run. This constructor is mainly called from the class
	 * BatchRunner.
	 * 
	 * @param defaultModel
	 *            the model to run an experiment with
	 * @param defaultExpRunner
	 *            the experiment runner used for running the experiment
	 * @param defaultExpSettings
	 *            the experiment parameter settings
	 * @param defaultModelParams
	 ...
	 * 
	 * @param n
	 *            the DOM node to read the run from
	 */
	...
	/**
	 * Creates a default desmoj.util.ExperimentRunner. This method might be
	...
	public ExperimentRunner getExperimentRunner() {
		if (model == null)
			...
			if (expname == null) {
				expname = (String) model.getName() + "Experiment" + "_"
						+ number;
	...
	/**
	 * Reads a parameter list from a DOM node to a java.util.Map. In the map
	 * each setting is stored as a key (name of the parameter) and value pair.
	 * 
	 * @param src
	 *            a DOM node containing XML based parameter settings
	 * @param dst
	 *            a java.uztil.Map to copy the parameter settings to.
	 */
