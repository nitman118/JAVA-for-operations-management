...
public class BackgroundElementGrafic extends JComponent implements Grafic{
	...
	private Point				topLeftIntern, topLeftExtern; 
	private Point				bottomRightIntern, bottomRightExtern;	// Position (1. Alternative)
	private Point				middlepointIntern, middlepointExtern;			// Position (2. Alternative)
	 ...
	 * @return 			smallest Rectangle, when an background element exist.
	 * 					null otherwise 
	 */
	...
	public static void updateInit(Model model, String viewId, JComponent panel){
		...
		for(int i=0; i<id.length; i++){
			...
			java.util.TreeSet<String> tmpSet = tmp.get(level);
			if(tmpSet == null) tmpSet = new java.util.TreeSet<String>();
			tmpSet.add(id[i]);
		...
		while(enLevel.hasNext()){
			java.util.TreeSet<String> tmpSet = tmp.get(enLevel.next());
			...
			while(enId.hasNext()){
				...
				if(backgroundElementGrafic != null && 
					...
					backgroundElementGrafic.update(backgroundElementGrafic.textColor, backgroundElementGrafic.background);
	...
	public BackgroundElementGrafic(BackgroundElement backgroundElement, String viewId, Point topLeft, Point bottomRight, 
		...
		if(this.sizeExtern == null) this.sizeExtern = this.contentLabel.getPreferredSize();
		if(this.sizeExtern == null) this.sizeExtern = defaultSize;
	...
	/**
	 * paint background element as JLayeredPane with a JLabel for 
	 * background-image and a JLabel for text
	 * Works only when sizeIntern is known.
	 * @return
	 */
	private JLayeredPane buildContent(){
		...
		if(size == null)	size = textLabel.getPreferredSize();
	...
	private void setBounds(){
		...
		if(this.getBorder() != null) in = this.getBorder().getBorderInsets(this);
	...
	private void setPreferredSize(){
		...
		Dimension d = new Dimension(this.sizeIntern.width+in.left+in.right, this.sizeIntern.height+in.top+in.bottom);
	...
	public Rectangle getBoundsExtern(){
		...
		if(this.middlepointExtern != null){
			if(this.sizeExtern != null) d = this.sizeExtern;
			else						d = this.sizeIntern;
