...
public class ResourceGrafic extends JPanel implements Grafic{
	...
	private JScrollPane 	queuePane, processPane;
	private JPanel 			queueFlow, queuePanel;
	private JPanel			processFlow, processPanel;
	private JLabel 			queueLength, processLength, usedRes;
	 ...
	 * @return 	smallest rectangle when an ResourceGrafic exist,
	 * 			null otherwise.
	 */
	...
	public ResourceGrafic(Resource resource, String viewId, Point pointExtern, 
		...
		if(resource.getName() != null) 	this.code = this.resource.getName();
		else 						this.code = "id: "+this.resource.getId();
		...
		JPanel cornerQ = new JPanel(); cornerQ.setLayout(new GridLayout(1,1)); cornerQ.setBackground(Grafic.COLOR_BACKGROUND);
		...
		if(this.horizontal){
			...
			legendeQ.add(legendeQ1Label); legendeQ.add(legendeQ2Label);
			...
			legendeP.add(legendeP1Label); legendeP.add(legendeP2Label);
			
			this.queuePane = new JScrollPane(this.queueFlow, JScrollPane.VERTICAL_SCROLLBAR_NEVER, JScrollPane.HORIZONTAL_SCROLLBAR_AS_NEEDED);
			...
			this.processPane = new JScrollPane(this.processFlow, JScrollPane.VERTICAL_SCROLLBAR_NEVER, JScrollPane.HORIZONTAL_SCROLLBAR_AS_NEEDED);
			...
			this.queuePane = new JScrollPane(this.queueFlow, JScrollPane.VERTICAL_SCROLLBAR_AS_NEEDED, JScrollPane.HORIZONTAL_SCROLLBAR_NEVER);
			...
			this.processPane = new JScrollPane(this.processFlow, JScrollPane.VERTICAL_SCROLLBAR_AS_NEEDED, JScrollPane.HORIZONTAL_SCROLLBAR_NEVER);
