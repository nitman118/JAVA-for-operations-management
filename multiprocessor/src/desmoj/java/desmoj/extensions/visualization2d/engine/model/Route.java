 ...
 * The line has a length and an entity can move on it with 
 * a velocity stored in its velocity-attribute 
 * 
...
public class Route extends Container<Long>{
	...
	public Route(Model model, String id, String sourceId, String sinkId, double length){
		...
		if(this.id != null) model.getRoutes().add(this);
	}
	 ...
	 * The endpoints are given from source- and sink- station
	 * @param points
	 * @return RouteGrafic
	 */
	public desmoj.extensions.visualization2d.engine.modelGrafic.Grafic createGrafic(Point[] points){
		...
		this.grafic = new desmoj.extensions.visualization2d.engine.modelGrafic.RouteGrafic(this, extPoints, this.getModel().getSimulationTime().getSimulationTime());
		return this.grafic;
	}
	...
	public boolean addToContainer(String entityId, long time)throws ModelException{
		...
		if(out){
			...
			throw new ModelException("model.Route.addToContainer It was not possible to add entity: "+entityId+" to Route: "+this.getId() );
		}
	...
	public boolean removeFromContainer(String entityId, long time)throws ModelException{
		...
		if(out){
			model.getEntities().get(entityId).changeContainer("Route", this.getId(), "remove at "+df.format(relPos), time);
			model.getEntities().get(entityId).changeContainer(" ", "free", "", time);
		}else{
			throw new ModelException("model.Route.removeFromContainer It was not possible to remove entity: "+entityId+" from Route: "+this.getId() );
		}
		if(this.grafic != null) this.grafic.update(time);
		return out;
	}
	
	/**
	 * get rel position of entityId on route. 
	 * 0.0 means entity is on source and 1.0 means entits is on sink.
	 * @param entityId
	 * @param time		time of operation
	 * @return rel position
	 */
	 ...
	 * This is a value between 0 (start of route) and 1 (end of route).
	 * @param time actual simulation time
	 * @return getRelPosition(long time)[i][0]: key of i.th entity in route (String).
	 *         getRelPosition(long time)[i][1]: relative Position of i.th entity in route (Double).
	 */
