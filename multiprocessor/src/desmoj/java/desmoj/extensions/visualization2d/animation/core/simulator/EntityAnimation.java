 ...
 * Free and static entities are supported. 
 * Free Entities can be included in Containers, as Queues, Processes, ect.
 * and have no fixed location.
 * Static Entities have a fixed location and can not be included in 
 * Containers, as Queues, Processes, ect.
 * 
...
public class EntityAnimation extends Entity implements EntityBasicAnimation{
	 ...
	 * @param showInTrace
	 */
	public EntityAnimation(ModelAnimation owner, String name,
			boolean showInTrace) {
	...
	public void createAnimation(String entityTypeId, boolean showInAnimation) {
		this.createAnimation(entityTypeId, "active", null, showInAnimation);
	}
	
	/**
	 * createAnimation method of free entities. These are non static entities 
	 * and can be included in Containers, such as Queues, Processes, ect. 
	 * @param entityTypeId		entitType of Entity 
	...
	/**
	 * createAnimation method of static entities. These entities have a fixed location, 
	 * and can not be part of a Container. 
	 * @param entityTypeId		entitType of Entity 
	 * @param state				initial state
	...
	public void disposeAnimation(){
		...
		if(this.showInAnimation){
			try {
				if(init)	c = Command.getCommandInit("disposeEntity", this.cmdGen.getAnimationTime(simTime));
				else 		c = Command.getCommandTime("disposeEntity", this.cmdGen.getAnimationTime(simTime));
				c.addParameter("EntityId", this.getName());
				c.setRemark(this.getGeneratedBy(EntityAnimation.class.getSimpleName()));
				cmdGen.checkAndLog(c);
				cmdGen.write(c);
	...
	public void setAttribute(String key, String value){
		...
		if(this.showInAnimation){
			try {
				if(init)	c = Command.getCommandInit("setEntity", this.cmdGen.getAnimationTime(simTime));
				else 		c = Command.getCommandTime("setEntity", this.cmdGen.getAnimationTime(simTime));
				c.addParameter("EntityId", this.getName());
	 ...
	 * when a entity moves on a route, velocity attribute describes the speed.
	 * Default attribute is velocity.
	 * @param attributeKey
	 */
